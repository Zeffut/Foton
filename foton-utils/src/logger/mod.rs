...
/// A reference to the Foton Logger\
/// Use the log macros instead: `log::info`!, `logger::chat`!, etc...
pub static FOTON_LOGGER: OnceLock<Arc<dyn FotonLogger>> = OnceLock::new();

/// Levels of logging in Foton
pub enum Level {
...
macro_rules! console {
    ($($arg:tt)+) =>
        ($crate::logger::FOTON_LOGGER.get().expect("Foton logger isn't initialized!").log(
...
/// A message visitor for the Foton Logger
#[derive(Default)]
pub struct LogData {
    /// The log message
    pub message: String,
...
/// A trait for Logging Foton data
pub trait FotonLogger: Send + Sync {
