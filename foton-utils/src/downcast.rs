//! Deterministic concrete-type downcasting for erased Foton objects.
...
/// `foton:entity/item`. They identify Rust implementations, not Minecraft
...
macro_rules! impl_foton_downcast_type {
    ($type:ty, $key:literal) => {
        // SAFETY: This Foton-owned key uniquely identifies the concrete Rust
...
impl_foton_downcast_type!((), "foton:rust_primitive/unit");
impl_foton_downcast_type!(bool, "foton:rust_primitive/bool");
impl_foton_downcast_type!(i32, "foton:rust_primitive/i32");
impl_foton_downcast_type!(f32, "foton:rust_primitive/f32");
impl_foton_downcast_type!(crate::Identifier, "foton:value/identifier");
impl_foton_downcast_type!(text_components::TextComponent, "foton:value/text_component");
...
mod tests {
    ...
    unsafe impl DowncastType for First {
        const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:test/downcast/first");
    }

    // SAFETY: These test-only keys are distinct and identify their respective
    // concrete types within the test process.
    unsafe impl DowncastType for Second {
        const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:test/downcast/second");
    }
