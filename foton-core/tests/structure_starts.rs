...
//!
//! Verifies that Foton's `create_structures` matches vanilla Minecraft for the
...
use foton_registry::template_pool::{PoolElement, ProcessorList, Projection};
use foton_utils::Rotation;
use foton_utils::{ChunkPos, Direction, Identifier};
...
impl ExpectedBoundingBox {
    const fn matches(&self, actual: &foton_utils::BoundingBox) -> bool {
...
/// Format a foton `BoundingBox` for inclusion in error messages.
fn fmt_bb_actual(bb: &foton_utils::BoundingBox) -> String {
    format!(
        "[{},{},{} .. {},{},{}]",
        bb.min_x(),
        bb.min_y(),
