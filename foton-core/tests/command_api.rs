//! Verifies the command extension surface from outside the `foton-core` crate.

use foton_core::command::{
...
unsafe impl DowncastType for NegatedBooleanParser {
    const TYPE_KEY: DowncastTypeKey =
        DowncastTypeKey::new("foton_test:command/parser/negated_bool");
}
...
unsafe impl DowncastType for NegatedBoolean {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton_test:command/value/negated_bool");
}
...
fn downstream_commands_can_register_primitive_keyed_and_suspended_nodes() {
    ...
    let command = CommandRegistration::new(Identifier::new("foton_test", "extension"), || {
        literal("extension")
            .then(
    ...
    assert!(registry.declare_permission("foton_test.extra").is_ok());
