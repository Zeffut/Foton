...
use foton_protocol::packets::game::{CMapItemData, MapDecorationData, MapPatch};
use foton_registry::data_components::components::MapDecorations;
use foton_registry::map_decoration_type::MapDecorationTypeRef;
use foton_registry::vanilla_map_decoration_types as decoration_types;
...
///
/// Vanilla reaches into the `Player` object directly. Foton snapshots the
...
///
/// Vanilla walks its `carriedBy` list of live `Player` references; Foton keeps
...
pub struct MapItemSavedData {
    ...
    /// arrows spin. Foton's world keys are `domain:world` and carry no vanilla
...
impl MapItemSavedData {
    ...
    /// one for a tick. Foton re-examines the shifted element instead.
