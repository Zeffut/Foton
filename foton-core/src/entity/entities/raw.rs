...
use foton_utils::{DowncastType, DowncastTypeKey, UuidExt, locks::SyncMutex};
use glam::DVec3;
...
/// Foton-specific fallback for entity types whose runtime behavior is not implemented yet.
///
/// Vanilla has concrete classes for every entity type. Foton uses this only to preserve
...
// SAFETY: This key identifies the Foton fallback implementation, independently
// of the Minecraft entity registry entry stored inside it.
unsafe impl DowncastType for RawEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/raw");
}

impl RawEntity {
    /// Creates a fresh raw entity for an entity type Foton cannot behaviorally model yet.
    #[must_use]
