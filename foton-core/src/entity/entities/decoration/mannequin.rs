...
//! Foton's `Player` predates that split, so the two are separate here; the
...
//! how a placed mannequin item carries a skin onto the entity. Foton's spawn
...
use foton_registry::entity_data::{EntityPose, HumanoidArm};
use foton_registry::entity_type::EntityTypeRef;
use foton_registry::resolvable_profile::ResolvableProfile;
use foton_registry::vanilla_entity_data::MannequinEntityData;
...
// SAFETY: This key is owned by Foton and uniquely identifies `MannequinEntity`.
unsafe impl DowncastType for MannequinEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/mannequin");
}
