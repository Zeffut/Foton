...
//! the first one in Foton that is not a mob -- and almost everything it does is
...
//! and the `Pose` tag is neither saved nor loaded -- Foton has no NBT codec for
...
use foton_registry::vanilla_entity_data::ArmorStandEntityData;
use foton_registry::{sound_events, vanilla_items};
...
// SAFETY: This key is owned by Foton and uniquely identifies `ArmorStandEntity`.
unsafe impl DowncastType for ArmorStandEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/armor_stand");
}
...
impl ArmorStandEntity {
    ...
    fn break_apart(&self, drop_the_stand: bool) {
        ...
        // Foton has no enchantment effect components, so every piece drops.
        let above = foton_utils::BlockPos::new(pos.x(), pos.y() + 1, pos.z());
...
impl LivingEntity for ArmorStandEntity {
    ...
    ///
    /// Not implemented: Foton has no rideable-minecart predicate reachable from
