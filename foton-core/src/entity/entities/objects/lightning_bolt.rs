...
//! `CHANNELED_LIGHTNING`. Both are advancement triggers and Foton has no
...
use foton_registry::{REGISTRY, level_events, vanilla_damage_types, vanilla_game_events};
...
unsafe impl DowncastType for LightningBoltEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/lightning_bolt");
}
...
impl LightningBoltEntity {
    ...
    /// lightning-strike advancement trigger Foton does not have yet.
    #[must_use]
...
mod tests {
    ...
    use foton_registry::vanilla_game_rules::FIRE_SPREAD_RADIUS_AROUND_PLAYER;
