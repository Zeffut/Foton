...
use foton_registry::vanilla_entity_data::FireballEntityData;
...
// SAFETY: This key is owned by Foton and uniquely identifies `LargeFireballEntity`.
unsafe impl DowncastType for LargeFireballEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/large_fireball");
}
...
impl LargeFireballEntity {
    ...
    ///
    /// Deviation from vanilla, from Foton's shared explosion API: the blast is
...
impl Projectile for LargeFireballEntity {
    ...
    fn on_hit_entity(&self, entity: &SharedEntity, _location: DVec3) {
        ...
        // Foton has no post-attack enchantment dispatch for projectiles yet.
    }
...
mod tests {
    ...
    use foton_utils::{ChunkPos, Direction};
    use glam::DVec3;
