...
use foton_registry::vanilla_entity_data::SpectralArrowEntityData;
use foton_registry::{
...
// SAFETY: This Foton-owned key uniquely identifies `SpectralArrowEntity`.
unsafe impl DowncastType for SpectralArrowEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/spectral_arrow");
}
...
impl SpectralArrowEntity {
    ...
    /// shot is the one that comes back; Foton builds it fresh, which is the
...
impl Projectile for SpectralArrowEntity {
    ...
    fn on_hit_block(&self, hit: &ClipHitResult) {
        ...
        // Foton's projectile engine does not advance a projectile to its hit
