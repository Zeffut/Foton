...
//! the parts of `AbstractArrow` Foton supports. Piercing, crit arrows and the
...
use foton_registry::vanilla_entity_data::TridentEntityData;
...
// SAFETY: This Foton-owned key uniquely identifies `ThrownTridentEntity`.
unsafe impl DowncastType for ThrownTridentEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/thrown_trident");
}
...
impl ThrownTridentEntity {
    ...
    fn set_pickup_item_stack(&self, stack: ItemStack) {
        ...
        // component; Foton has no such component yet, so any enchantment glints.
        let foil = stack
            .get_enchantments()
    ...
    fn tick_loyalty_return(&self) -> bool {
        ...
        if !Self::is_acceptible_return_owner(&owner) {
            ...
            // discarding. Foton's removal is immediate, so ticking the arrow
    ...
    fn abstract_arrow_tick(&self) {
        ...
        if self.is_in_ground() && physics_enabled {
            ...
            // loose when the block it is stuck in disappears. Foton's arrow does
...
impl Entity for ThrownTridentEntity {
    ...
    fn save_additional(&self, nbt: &mut NbtCompound) {
        ...
        // `PierceLevel`, `SoundEvent` and `weapon` are not modeled by Foton's
...
impl Projectile for ThrownTridentEntity {
    ...
    fn on_hit_entity(&self, entity: &SharedEntity, _location: DVec3) {
        ...
        if entity.hurt(&world, &source, damage) {
            ...
            // `LightningBolt`, an entity Foton does not implement, so the helper
    ...
    fn on_hit_block(&self, hit: &ClipHitResult) {
        ...
        // `minecraft:hit_block` enchantment effects, and Foton has no application
