...
// SAFETY: This key is owned by Foton and uniquely identifies `SpawnerMinecartEntity`.
unsafe impl DowncastType for SpawnerMinecartEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/spawner_minecart");
}
...
impl SpawnerOwner for SpawnerMinecartEntity {
    ...
    ///
    /// Foton's `EntityStatus` is generated from vanilla's `EntityEvent`
    ...
    /// the server depends on it and Foton does not send it.
