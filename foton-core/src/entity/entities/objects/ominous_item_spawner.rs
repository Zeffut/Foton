...
use foton_registry::vanilla_entity_data::OminousItemSpawnerEntityData;
use foton_registry::{level_events, sound_events, vanilla_entities, vanilla_game_events};
...
// SAFETY: This key is owned by Foton and uniquely identifies `OminousItemSpawnerEntity`.
unsafe impl DowncastType for OminousItemSpawnerEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/ominous_item_spawner");
}
...
impl OminousItemSpawnerEntity {
    ...
    /// than drop them. Foton has no `ProjectileItem` dispatch and no
