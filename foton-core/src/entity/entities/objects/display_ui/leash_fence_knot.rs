...
// SAFETY: This key is owned by Foton and uniquely identifies `LeashFenceKnotEntity`.
unsafe impl DowncastType for LeashFenceKnotEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/leash_fence_knot");
}
...
impl LeashFenceKnotEntity {
    ...
    /// knot unconditionally because `addFreshEntity` cannot fail; Foton returns
