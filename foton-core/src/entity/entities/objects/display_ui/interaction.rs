...
use foton_utils::{DowncastType, DowncastTypeKey, UuidExt};
use glam::DVec3;
...
// SAFETY: This key is owned by Foton and uniquely identifies `InteractionEntity`.
unsafe impl DowncastType for InteractionEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/interaction");
}
...
impl Entity for InteractionEntity {
    ...
    /// here. Foton has no advancement criteria system, so that trigger is not
    /// raised.
