...
use foton_registry::painting_variant::PaintingVariantRef;
use foton_registry::vanilla_entity_data::PaintingEntityData;
use foton_registry::vanilla_game_rules::ENTITY_DROPS;
use foton_registry::vanilla_painting_variant_tags::PaintingVariantTag;
use foton_registry::{
...
unsafe impl DowncastType for PaintingEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/painting");
}
...
impl PaintingEntity {
    ...
    ///
    /// Vanilla parity: `HangingEntity.survives`. Foton's item frame has no
    ...
    /// crosses the world border. Foton's `WorldCollisionProvider` only reports
    ...
    /// Foton has no `HangingEntity` supertype, so the concrete hanging
...
/// Vanilla parity: `PaintingVariant.area`, which Foton's registry value does
/// not carry.
...
/// The facing of `entity` when it is one of Foton's hanging entities.
///
/// Vanilla parity: the `EntityTypeTest.forClass(HangingEntity.class)` of
/// `HangingEntity.canCoexist`. Foton has no `HangingEntity` supertype, so the
...
#[cfg(test)]
mod tests {
    use foton_registry::{init_vanilla_registry, vanilla_entities, vanilla_painting_variants};
