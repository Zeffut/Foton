...
use foton_registry::{REGISTRY, RegistryExt as _, vanilla_blocks, vanilla_villager_professions};
...
/// at. Foton registers one `WorkAtPoi` on every villager and has it look at the
...
/// Foton's composter block behavior does not implement. A farmer therefore
