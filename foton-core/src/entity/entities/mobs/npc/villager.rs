...
use foton_registry::trading::{MerchantOffers, TradeSet, offer_nbt};
use foton_registry::vanilla_entity_data::VillagerEntityData;
use foton_registry::vanilla_item_tags::ItemTag;
use foton_registry::vanilla_poi_type_tags::PoiTag;
use foton_registry::villager_profession::VillagerProfessionRef;
use foton_registry::villager_type::{VillagerType, VillagerTypeRef};
use foton_registry::{
...
use foton_utils::{Downcast as _, DowncastType, DowncastTypeKey, GlobalPos, Identifier};
use glam::DVec3;
...
// SAFETY: This key is owned by Foton and uniquely identifies `VillagerEntity`.
unsafe impl DowncastType for VillagerEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/villager");
}
...
impl VillagerEntity {
    ...
    /// rebuilds the brain; Foton's brain outlives growing up, so this is called
    ...
    /// biome lookup always answers; Foton's can miss when the column is not
...
impl AgeableMob for VillagerEntity {
    ...
    /// Foton registers both packages on every villager and only has to swap the
...
impl Mob for VillagerEntity {
    ...
    /// generated village has a trade the moment it lands. Foton generates no
