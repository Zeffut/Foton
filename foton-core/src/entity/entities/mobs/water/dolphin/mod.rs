...
//! pos, 50, false)`, and Foton has no structure-locating call at all -- nothing
//! under `foton-worldgen` or `foton-core` answers "where is the nearest
...
use foton_registry::vanilla_entity_data::DolphinEntityData;
...
// SAFETY: This key is owned by Foton and uniquely identifies `DolphinEntity`.
unsafe impl DowncastType for DolphinEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/dolphin");
}
...
impl DolphinEntity {
    ...
    fn new_with_base(base: EntityBase, entity_type: EntityTypeRef) -> Self {
        ...
        // is not ported: Foton has no `Mob.pickUpItem` hook, so nothing walks a
        ...
        *mob_base.can_pick_up_loot().lock() = true;
...

        {
            ...
            // Vanilla also escorts an `AbstractNautilus`; Foton has no nautilus
        ...
        }
        {
            ...
            // retaliate against a guardian, it flees one. Foton has no guardian
...
impl Entity for DolphinEntity {
    ...
    // Vanilla `Dolphin.getSwimSplashSound` returns `DOLPHIN_SPLASH`. Foton has
