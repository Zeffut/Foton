...
//! guardian spawn inside the sea at all. Foton's spawn path has no
...
// SAFETY: This key is owned by Foton and uniquely identifies `GuardianEntity`.
unsafe impl DowncastType for GuardianEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/guardian");
}
