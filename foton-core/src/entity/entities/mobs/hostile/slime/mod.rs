...
use foton_registry::vanilla_entity_data::SlimeEntityData;
...
use foton_utils::{BlockPos, ChunkPos, DowncastType, DowncastTypeKey};
...
///
/// TODO: read the per-position environment attribute once Foton has them; this
...
// SAFETY: This key is owned by Foton and uniquely identifies `SlimeEntity`.
unsafe impl DowncastType for SlimeEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/slime");
}
...
impl Entity for SlimeEntity {
    ...
    /// Vanilla parity: `Slime` adds nothing to
...
impl LivingEntity for SlimeEntity {
    ...
    /// behind. Foton has a death hook and no removal hook, so the split hangs
