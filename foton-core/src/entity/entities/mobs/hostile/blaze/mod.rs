...
//! a blaze reads every block as fully lit when it scores a walk target. Foton's
...
use foton_registry::vanilla_entity_data::BlazeEntityData;
use foton_registry::{level_events, sound_events, vanilla_attributes, vanilla_entities};
...
// SAFETY: This key is owned by Foton and uniquely identifies `BlazeEntity`.
unsafe impl DowncastType for BlazeEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/blaze");
}
