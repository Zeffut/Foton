...
use foton_registry::vanilla_entity_data::SkeletonEntityData;
...
///
/// Vanilla parity: `AbstractSkeleton.getHardAttackInterval`.
const HARD_ATTACK_INTERVAL_TICKS: i32 = 20;

/// Ticks between shots on every other difficulty.
///
/// Vanilla parity: `AbstractSkeleton.getAttackInterval`.
const ATTACK_INTERVAL_TICKS: i32 = 40;
...
// SAFETY: This key is owned by Foton and uniquely identifies `SkeletonEntity`.
unsafe impl DowncastType for SkeletonEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/skeleton");
}
...
impl Entity for SkeletonEntity {
    ...
    /// Vanilla parity: the `Mob` half of `Skeleton.addAdditionalSaveData`.
    /// Vanilla also writes `StrayConversionTime`; Foton has no powder-snow
    /// freeze conversion yet, so there is no timer to write.
