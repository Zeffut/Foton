...
use foton_registry::data_components::components::SulfurCubeContent;
use foton_registry::data_components::vanilla_components;
use foton_registry::entity_data::EntityPose;
use foton_registry::entity_type::{EntityAttachment, EntityDimensions, EntityTypeRef};
use foton_registry::item_stack::ItemStack;
use foton_registry::item_stack_template::ItemStackTemplate;
use foton_registry::sound_event::SoundEventRef;
use foton_registry::sulfur_cube_archetype::{
...
use foton_registry::vanilla_entity_data::SulfurCubeEntityData;
use foton_registry::vanilla_game_rules::{MOB_GRIEFING, TNT_EXPLODES, TNT_EXPLOSION_DROP_DECAY};
...
use foton_utils::random::xoroshiro::Xoroshiro;
use foton_utils::types::InteractionHand;
use foton_utils::{BlockPos, ChunkPos, Downcast as _, DowncastType, DowncastTypeKey};
use glam::DVec3;
...
// SAFETY: This key is owned by Foton and uniquely identifies `SulfurCubeEntity`.
unsafe impl DowncastType for SulfurCubeEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/sulfur_cube");
}
...
impl SulfurCubeEntity {
    ...
    ///
    /// Foton gap: `bounciness`, `friction_modifier` and `air_drag_modifier` are
    /// applied here as vanilla writes them, but Foton's movement code does not
    ...
    fn apply_contact_damage(&self, target: &SharedEntity) {
        ...
        // Foton's runtime RNG instead of being carried on the entity.
    ...
    /// Vanilla parity: the flint-and-foton branch of `SulfurCube.mobInteract`.
    fn try_light(
        &self,
        player: &Player,
        hand: InteractionHand,
        ...
        if !tnt_explodes {
            ...
            // here; Foton's overlay message path is the same either way, and
...
///
/// Foton gap: there is no `AbstractArrow` capability trait to narrow this to
...
impl LivingEntity for SulfurCubeEntity {
    ...
    ///
    /// Foton gap: vanilla answers the immune case with
    /// `dealDefaultKnockback(source, damage, true)`, which needs the damage
    /// amount that Foton's `LivingEntity::knockback` does not carry. The
