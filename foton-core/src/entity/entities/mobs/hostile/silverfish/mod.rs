...
use foton_registry::vanilla_entity_data::SilverfishEntityData;
...
/// placement predicate. Foton's natural spawner
/// (`foton-core/src/world/natural_spawn.rs`) has no per-entity-type
...
// SAFETY: This key is owned by Foton and uniquely identifies `SilverfishEntity`.
unsafe impl DowncastType for SilverfishEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/silverfish");
}
...
impl Entity for SilverfishEntity {
    ...
    /// Vanilla parity: `Silverfish` inherits `Mob.addAdditionalSaveData`
...
impl LivingEntity for SilverfishEntity {
    ...
    /// `LivingEntity.hurtServer`. Foton's `LivingEntity::hurt_server` default
...
impl PathfinderMob for SilverfishEntity {
    ...
    /// Foton's `PathfinderMob::get_walk_target_value` default only
...
mod tests {
    ...
    fn downcast_key_identifies_silverfish() {
        assert_eq!(
            ...
            DowncastTypeKey::new("foton:entity/silverfish")
        );
    }
