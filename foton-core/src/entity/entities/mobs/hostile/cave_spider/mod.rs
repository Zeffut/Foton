...
use foton_registry::vanilla_entity_data::CaveSpiderEntityData;
...
// SAFETY: This key is owned by Foton and uniquely identifies `CaveSpiderEntity`.
unsafe impl DowncastType for CaveSpiderEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/cave_spider");
}
...
impl Entity for CaveSpiderEntity {
    ...
    /// Vanilla parity: neither `CaveSpider` nor `Spider` overrides
