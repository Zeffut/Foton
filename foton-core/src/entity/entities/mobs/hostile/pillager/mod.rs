...
use foton_registry::vanilla_entity_data::PillagerEntityData;
...
// SAFETY: This key is owned by Foton and uniquely identifies `PillagerEntity`.
unsafe impl DowncastType for PillagerEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/pillager");
}
...
impl PillagerEntity {
    ...
    fn new_with_base(base: EntityBase, entity_type: EntityTypeRef) -> Self {
...

        {
            ...
            // walk the village. Foton has no creaking and no raid.
        }
    ...
    /// `enchantSpawnedWeapon`. Foton has no enchantment providers, so the
    ...
    fn populate_default_equipment(&self) {
        ...
        if rand::random_range(0..SPAWN_ENCHANT_DENOMINATOR) == 0 {
            ...
            // once Foton has enchantment providers.
        }
    }
}
...
///
/// Foton's `CrossbowItem` loads and fires from a player's inventory, so the
/// bolt is not drawn from the mob's ammunition and Piercing on the weapon is
/// not applied. Both are the same gap Foton's crossbow already documents.
...
impl Raider for PillagerEntity {
    ...
    /// nothing at all below. Foton has no enchantment providers, so the
