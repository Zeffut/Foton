...
//! land, switching between the two as it goes. Foton decides that per path
...
use foton_registry::vanilla_entity_data::DrownedEntityData;
use foton_registry::vanilla_item_tags::ItemTag;
use foton_registry::{REGISTRY, TaggedRegistryExt as _, sound_events, vanilla_items};
...
// SAFETY: This key is owned by Foton and uniquely identifies `DrownedEntity`.
unsafe impl DowncastType for DrownedEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/drowned");
}
    ...
    fn new() -> Self {
        Self {
            inner: NearestAttackableTargetGoal::new_for_players(true, |drowned, target, world| {
                // Vanilla captures `this` here; Foton passes the searcher in.
                let _ = drowned;
                !world.is_bright_outside() || target.is_in_water()
            }),
        }
...
impl Mob for DrownedEntity {
    ...
    /// and husk siblings Foton only syncs the flag so far, without vanilla's
...
impl PathfinderMob for DrownedEntity {
    ...
    ///
    /// Vanilla parity: `AmphibiousPathNavigation`, which Foton expresses by
