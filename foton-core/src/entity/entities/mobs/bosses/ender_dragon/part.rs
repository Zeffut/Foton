...
//! so that a part counts as its parent in identity tests. Foton implements
...
pub struct EnderDragonPart {
    ...
    ///
    /// Vanilla holds `parentMob` directly. Foton cannot: the dragon's `Arc`
...
// SAFETY: This key is owned by Foton and uniquely identifies `EnderDragonPart`.
unsafe impl DowncastType for EnderDragonPart {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/ender_dragon_part");
}
