...
use foton_registry::data_components::vanilla_components::{CONSUMABLE, FOOD};
...
use foton_registry::vanilla_entity_data::FoxEntityData;
...
use foton_utils::{BlockPos, Downcast as _, DowncastType, DowncastTypeKey, UuidExt};
use glam::DVec3;
...
// SAFETY: This key is owned by Foton and uniquely identifies `FoxEntity`.
unsafe impl DowncastType for FoxEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/fox");
}
...
impl FoxEntity {
    ...
    fn register_goals(&self) {
        ...
        // wandering through a village. Foton has no village points of interest,
    ...
    ///
    /// Foton matches on the spawn category rather than a Java class, which is
    ...
    pub fn pick_berries(&self, pos: BlockPos) {
        ...
        if state.get_block() != &vanilla_blocks::SWEET_BERRY_BUSH {
            ...
            // berries. Foton's cave-vine behavior exposes no server-side pick
...
impl Mob for FoxEntity {
    ...
    fn finalize_spawn(
        ...
        // and turns members past the first into kits. Foton's `SpawnGroupData`
    ...
    /// from a spawn egg trust the player who used it. Foton's `Mob.interact`
