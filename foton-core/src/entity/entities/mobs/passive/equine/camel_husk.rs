...
//! [`super::camel_common`].
//!
//! One vanilla override has no reader in Foton yet and is deliberately absent:
//! `chargeSpeedModifier`, which returns `4.0F` so a rider charging with a spear
//! is carried four times as fast. Foton has no `SpearUseGoal`, `SpearAttack`,
...
use foton_registry::vanilla_entity_data::CamelHuskEntityData;
...
// SAFETY: This key is owned by Foton and uniquely identifies `CamelHuskEntity`.
unsafe impl DowncastType for CamelHuskEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/camel_husk");
}
