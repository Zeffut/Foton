//! The vanilla mule.
//!
//! Vanilla parity: `net.minecraft.world.entity.animal.equine.Mule`. The cross a
//! horse and a donkey make: a chested horse that cannot pass its own line on,
//! because it never overrides `AbstractHorse.canMate`.
...
use foton_registry::vanilla_entity_data::MuleEntityData;
...
///
/// Vanilla parity: the `scale(0.5F)` of the `AbstractChestedHorse` baby dimensions.
const CHESTED_BABY_SCALE: f32 = 0.5;

/// Where a mule foal carries its rider.
const MULE_BABY_PASSENGER_ATTACHMENTS: [EntityAttachmentPoint; 1] = [EntityAttachmentPoint::new(
    0.0,
    (1.6 + 0.031_25) * CHESTED_BABY_SCALE as f64,
    -0.312_5 * CHESTED_BABY_SCALE as f64,
)];

/// A mule foal's hitbox.
const MULE_BABY_DIMENSIONS: EntityDimensions = EntityDimensions::new_with_attachments(
    1.396_484_4 * CHESTED_BABY_SCALE,
    1.6 * CHESTED_BABY_SCALE,
    1.52 * CHESTED_BABY_SCALE,
    EntityAttachments::new(&MULE_BABY_PASSENGER_ATTACHMENTS, &[], &[], &[]),
);
...
// SAFETY: This key is owned by Foton and uniquely identifies `MuleEntity`.
unsafe impl DowncastType for MuleEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/mule");
}
...
impl MuleEntity {
    ...
    /// Creates a mule from saved base data.
    #[must_use]
...
impl Entity for MuleEntity {
    ...
    fn dimensions_for_pose(&self, _pose: EntityPose) -> EntityDimensions {
        ...
        if AgeableMob::is_baby(self) {
            MULE_BABY_DIMENSIONS.scale(scale)
...
impl Animal for MuleEntity {
    ...
    /// Vanilla parity: `AbstractHorse.canMate`, which a mule never overrides --
    /// this is why mules are a dead end.
    fn can_mate(&self, _partner: &dyn Animal) -> bool {
        false
    }

    /// Vanilla parity: `Mule.getBreedOffspring`, which -- unlike every other
    /// horse -- does not cross the parents' attributes into the foal.
...
impl AbstractHorse for MuleEntity {
    ...
    fn eating_sound(&self) -> Option<SoundEventRef> {
        Some(&sound_events::ENTITY_MULE_EAT)
    }

    fn angry_sound(&self) -> Option<SoundEventRef> {
        Some(&sound_events::ENTITY_MULE_ANGRY)
    }

    fn play_jump_sound(&self) {
        self.play_sound(&sound_events::ENTITY_MULE_JUMP, 0.4, 1.0);
    }
...
impl AbstractChestedHorse for MuleEntity {
    /// Vanilla parity: `Mule.playChestEquipsSound`.
    fn play_chest_equips_sound(&self) {
        let pitch = (rand::random::<f32>() - rand::random::<f32>()).mul_add(0.2, 1.0);
        self.play_sound(&sound_events::ENTITY_MULE_CHEST, 1.0, pitch);
    }
