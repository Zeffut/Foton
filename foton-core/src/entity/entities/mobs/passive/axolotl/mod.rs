...
use foton_registry::vanilla_entity_data::AxolotlEntityData;
...
// SAFETY: This key is owned by Foton and uniquely identifies `AxolotlEntity`.
unsafe impl DowncastType for AxolotlEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/axolotl");
}
...
impl Mob for AxolotlEntity {
    ...
    /// only ever spawns in water. Foton has no `checkSpawnObstruction` hook on
