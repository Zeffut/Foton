...
use foton_registry::vanilla_entity_data::SnifferEntityData;
...
// SAFETY: This key is owned by Foton and uniquely identifies `SnifferEntity`.
unsafe impl DowncastType for SnifferEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/sniffer");
}
...
impl Mob for SnifferEntity {
    ...
    /// Vanilla parity: `Sniffer.supportQuadLeash`.
...
impl PathfinderMob for SnifferEntity {
    ...
    // `onPathfindingDone` puts it back. Foton's navigation has no start/done
