...
use foton_registry::vanilla_entity_data::RabbitEntityData;
...
use foton_utils::{BlockPos, Downcast as _, DowncastType, DowncastTypeKey, Identifier};
use glam::DVec3;
...
// SAFETY: This key is owned by Foton and uniquely identifies `RabbitEntity`.
unsafe impl DowncastType for RabbitEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/rabbit");
}
...
impl Entity for RabbitEntity {
    ...
    // client from `Entity.spawnSprintParticle`, so Foton has nothing to skip.
...
/// `canRaid` as goal fields that its `isValidTarget` override writes; Foton
