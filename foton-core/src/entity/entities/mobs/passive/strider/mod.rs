...
use foton_registry::vanilla_entity_data::StriderEntityData;
...
// SAFETY: This key is owned by Foton and uniquely identifies `StriderEntity`.
unsafe impl DowncastType for StriderEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/strider");
}
...
impl StriderEntity {
    ...
    /// strider's feet clear its own half-height liquid collision shape; Foton
...
impl Mob for StriderEntity {
    ...
    /// created and mounted from inside `finalize_spawn`, and Foton calls this
...
mod tests {
    ...
    use foton_registry::{init_vanilla_registry, vanilla_entities, vanilla_fluids};
    ...
    fn downcast_key_identifies_strider() {
        assert_eq!(
            ...
            DowncastTypeKey::new("foton:entity/strider")
        );
    }
