...
use foton_registry::frog_variant::FrogVariantRef;
use foton_registry::item_stack::ItemStack;
use foton_registry::registry_reference::RegistryReference;
use foton_registry::sound_event::SoundEventRef;
use foton_registry::vanilla_block_tags::BlockTag;
use foton_registry::vanilla_entity_data::FrogEntityData;
...
// `BodyRotationControl`. Foton's body rotation has no head-rotation-speed seam
...
// SAFETY: This key is owned by Foton and uniquely identifies `FrogEntity`.
unsafe impl DowncastType for FrogEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/frog");
}
...
/// `Frog.canEat`. Foton has no shared cube-mob trait object, so the two cube
...
impl PathfinderMob for FrogEntity {
    ...
    /// Foton answers this per path request, the same seam the turtle and the
    /// drowned use.
    ///
    ...
    /// Foton's node evaluators are not per-mob, so a frog reaches lily pads
