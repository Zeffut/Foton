...
use foton_registry::vanilla_entity_data::TurtleEntityData;
...
use foton_utils::{BlockPos, BlockStateId, ChunkPos, Downcast as _, DowncastType, DowncastTypeKey};
...
// SAFETY: This key is owned by Foton and uniquely identifies `TurtleEntity`.
unsafe impl DowncastType for TurtleEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/turtle");
}
...
impl PathfinderMob for TurtleEntity {
    ...
    /// `AmphibiousPathNavigation`. Foton answers this per path request, the
