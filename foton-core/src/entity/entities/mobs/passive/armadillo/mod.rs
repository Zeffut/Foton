...
use foton_registry::vanilla_entity_data::ArmadilloEntityData;
...
// SAFETY: This key is owned by Foton and uniquely identifies `ArmadilloEntity`.
unsafe impl DowncastType for ArmadilloEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/armadillo");
}
...
impl ArmadilloEntity {
    ...
    ///
    /// Vanilla resets `inStateTicks` from `onSyncedDataUpdated`; Foton does it
    ...
    ///
    /// Foton has no loot tables, so the single-entry `brush/armadillo` table is
    ...
    fn tick_scute_shed(&self, world: &Arc<World>) {
        ...
        // which always pays out; Foton has no loot tables.
        self.spawn_at_location(ItemStack::new(&vanilla_items::ARMADILLO_SCUTE), 0.0);
        self.play_sound(
