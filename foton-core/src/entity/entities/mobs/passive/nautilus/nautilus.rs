...
use foton_registry::vanilla_entity_data::NautilusEntityData;
use foton_registry::{sound_events, vanilla_damage_types};
...
// SAFETY: This key is owned by Foton and uniquely identifies `NautilusEntity`.
unsafe impl DowncastType for NautilusEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/nautilus");
}
...
impl Mob for NautilusEntity {
    ...
    /// because it only ever spawns in water. Foton has no
