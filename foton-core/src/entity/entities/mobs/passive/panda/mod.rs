...
use foton_registry::vanilla_entity_data::PandaEntityData;
...
/// a sneeze pays out about one time in seven hundred. Foton has no loot tables,
...
pub struct PandaEntity {
    ...
    /// reach into it. Foton's goals live behind the selector's mutex, so the
    /// panda owns it and the look goal reads it.
    unhappy_look_target: SyncMutex<Option<i32>>,
    entity_data: SyncMutex<PandaEntityData>,
}

// SAFETY: This key is owned by Foton and uniquely identifies `PandaEntity`.
unsafe impl DowncastType for PandaEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/panda");
}
