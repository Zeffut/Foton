//! Wolf entity.
//!
//! Vanilla parity: `Wolf`. The first tameable mob in Foton, and the one that
...
use foton_registry::particle_type::{ItemParticleOption, ParticleData};
use foton_registry::sound_event::SoundEventRef;
use foton_registry::vanilla_block_tags::BlockTag;
use foton_registry::vanilla_entity_data::WolfEntityData;
use foton_registry::vanilla_entity_type_tags::EntityTypeTag;
use foton_registry::vanilla_item_tags::ItemTag;
use foton_registry::wolf_sound_variant::{WolfAge, WolfSoundVariantRef};
use foton_registry::wolf_variant::WolfVariantRef;
use foton_registry::{
...
///
/// Vanilla keeps four loose fields; they only ever change together, so Foton
...
// SAFETY: This key is owned by Foton and uniquely identifies `WolfEntity`.
unsafe impl DowncastType for WolfEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:entity/wolf");
}
...
impl WolfEntity {
    ...
    fn register_goals(&self) {
        ...
        // Turtle.BABY_ON_LAND_SELECTOR)` is not registered: Foton has no turtle
    ...
    fn tamed_interact(
        ...
        self.mob_base.navigation().lock().stop();
        self.set_target(None);
        // Vanilla returns `InteractionResult.SUCCESS.withoutItem()`; Foton has
...
///
/// Vanilla parity: `Wolf.WolfAvoidEntityGoal`. Foton has no llama yet, so the
...
impl Entity for WolfEntity {
    ...
    fn save_additional(&self, nbt: &mut NbtCompound) {
        ...
        if let Some(target) = self.persistent_anger_target() {
            nbt.insert(
                "angry_at",
                NbtTag::IntArray(foton_utils::UuidExt::to_int_array(&target).to_vec()),
            );
        }
    }
...
impl TamableAnimal for WolfEntity {
    ...
    fn wants_to_attack(&self, target: &dyn LivingEntity, owner: &dyn Entity) -> bool {
        ...
        // teammate of the owner. Foton has no scoreboard teams, and vanilla's
...
impl Mob for WolfEntity {
    ...
    fn finalize_spawn(
        ...
        // every wolf of one spawn group matches. Foton's `SpawnGroupData` has
