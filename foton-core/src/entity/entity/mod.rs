...
/// Concrete implementations must also claim a unique [`foton_utils::DowncastTypeKey`]
/// through [`foton_utils::DowncastType`].
///
/// # Using `EntityBase`
///
...
pub trait Entity: EntityEventSource + ErasedType + Send + Sync + 'static {
    ...
    /// Runs vanilla `Entity.baseTick` pieces Foton currently implements.
    ///
    ...
    /// Runs vanilla `Entity.handlePortal` behavior currently implemented by Foton.
    ...
    /// Foton calls it from the same three places.
    ...
    /// back a `SlotAccess`, a get-and-set pair over the slot; Foton's slots
    ...
    /// the commands Foton has that address slots by number ever do. `/item`
    ...
    /// `AbstractVillager`, then `LivingEntity` -- but in Foton each of those is
