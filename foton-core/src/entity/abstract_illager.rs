...
use foton_registry::{REGISTRY, TaggedRegistryExt as _, vanilla_entity_type_tags::EntityTypeTag};
...
/// pose. Foton keeps the derivation server-side anyway, because it is the one
...
pub trait AbstractIllager: Raider {
    ...
    /// entities to be teamless, which is free here: Foton puts no entity on a
    /// scoreboard team.
    ///
