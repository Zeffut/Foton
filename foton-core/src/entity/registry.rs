...
use foton_registry::{REGISTRY, RegistryEntry};
use glam::DVec3;
use simdnbt::borrow::{
...
impl EntityRegistry {
    ...
    /// to round-trip an entity Foton cannot model yet rather than lose it. A
...
mod tests {
    ...
    fn create_and_load_or_raw_preserves_unregistered_entity_data() {
        ...
        assert_eq!(
            saved.string("FotonRawMarker").map(ToString::to_string),
            Some("raw".to_owned())
        );
    }
