...
use foton_registry::{REGISTRY, TaggedRegistryExt as _, vanilla_attributes};
...
/// object out on the mob; Foton keeps one [`JumpControl`] in [`MobControls`] and
...
/// `Guardian.setMoving`; Foton returns it because the flag lives in the
...
pub struct GhastMoveControl {
    ...
    /// the control object; Foton's controls are recreated each tick, so the
