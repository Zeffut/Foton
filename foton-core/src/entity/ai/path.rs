...
use foton_utils::{BlockPos, BlockStateId, PackedBlockPos};
use glam::DVec3;
...
/// Vanilla `PathType`.
///
/// Foton stores per-mob overrides in a fixed array keyed by this enum instead
...
#[cfg(test)]
mod tests {
    use foton_registry::{REGISTRY, init_vanilla_registry, vanilla_blocks};
    ...
    impl LevelReader for SingleBlockLevel {
        fn get_block_state(&self, _pos: foton_utils::BlockPos) -> BlockStateId {
            self.state
        }

        fn raw_brightness(&self, _pos: foton_utils::BlockPos, _sky_darkening: u8) -> u8 {
            0
        }
    ...
    fn path_type_cache_invalidates_matching_position() {
        ...
        let pos = foton_utils::BlockPos::new(1, 64, 1);
    ...
    fn pathfinding_context_uses_cache_when_supplied() {
        ...
        let mut context = PathfindingContext::with_cache(
            &level,
            foton_utils::BlockPos::new(0, 64, 0),
            &mut cache,
        );
