...
//! is emptied. Foton had only the second half, and it spawned a fresh mob
//! rather than the one that went in; this is the first half plus the state the
//! second half was missing.

use std::io::Cursor;

use foton_registry::data_components::CustomData;
...
/// Foton reads the borrowed shape, so the tag makes one round trip through
