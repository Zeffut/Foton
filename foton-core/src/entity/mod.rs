...
use foton_registry::entity_data::{DataValue, EntityPose, HumanoidArm};
use foton_registry::entity_type::{EntityAttachment, EntityDimensions, EntityTypeRef};
use foton_registry::fluid::{FluidState, FluidStateExt as _};
...
use foton_registry::vanilla_game_rules::{MAX_ENTITY_CRAMMING, MOB_DROPS};
...
use foton_registry::{RegistryEntry, RegistryExt};
use foton_registry::{vanilla_attributes, vanilla_fluid_tags, vanilla_items, vanilla_mob_effects};
use foton_utils::entity_events::EntityStatus;
use foton_utils::locks::SyncMutex;
use foton_utils::types::{Difficulty, InteractionHand, UpdateFlags};
use foton_utils::{
...
/// necessarily receives the IDs right after its own. Foton builds entities off
...
macro_rules! impl_test_downcast_type {
    ($type:ty) => {
        ...
        unsafe impl foton_utils::DowncastType for $type {
            const TYPE_KEY: foton_utils::DowncastTypeKey = foton_utils::DowncastTypeKey::new(
                concat!("foton:test/", module_path!(), "/", stringify!($type)),
            );
        }
    };
}
...
pub fn start_riding_entities(passenger: &SharedEntity, entity_to_ride: &SharedEntity) -> bool {
    ...
    // TODO: emit the ENTITY_MOUNT game event here too, once Foton has one.
...
///
/// A loot [`EntityRef`] borrows its equipment, but Foton keeps equipment behind
