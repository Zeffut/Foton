...
use foton_registry::enchantment_effect::{
...
use foton_registry::{REGISTRY, RegistryExt, TaggedRegistryExt, vanilla_entities};
...
/// against a `LootContext` built from the shooter; Foton only builds
...
/// A requirement Foton's item-only context cannot decide fails closed, the same
