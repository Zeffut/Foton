...
pub use foton_registry::equipment::{EquipmentSlot, EquipmentSlotType};
...
/// a Foton [`Container`] hands out `&[ItemStack]`, so the storage itself has to
...
// SAFETY: This Foton-owned key uniquely identifies `OwnedEntityEquipment`.
unsafe impl DowncastType for OwnedEntityEquipment {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:container/entity_equipment");
}
