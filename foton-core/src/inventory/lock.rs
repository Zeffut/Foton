...
use foton_utils::{Downcast as _, DowncastType, locks::SyncMutex};
...
///
/// A double chest is two independently lockable halves in Foton, so a single
...
impl ContainerLockGuard {
        ...
        C: Borrow<ContainerRef>,
    {
        ...
        // unpacks it. Rolling here, before a single lock is taken, is Foton's
...
mod tests {
    ...
    use foton_utils::{BlockPos, ChunkPos, locks::SyncMutex};
