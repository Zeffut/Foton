...
//! `HorseInventoryMenu` and `NautilusInventoryMenu`. Foton builds one menu for
...
/// It is nothing but a cast to the concrete mount, so Foton passes the cast in
...
/// this menu itself, so Foton has nothing to send for it.
struct MountEquipmentSlot {
    base: ArmorSlot,
    mount: WeakEntity,
}

// SAFETY: This Foton-owned key uniquely identifies `MountEquipmentSlot`.
unsafe impl DowncastType for MountEquipmentSlot {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:slot/mount_equipment");
}
...
unsafe impl DowncastType for MountKind {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:menu/mount");
}
...
impl MenuKind for MountKind {
    ...
    fn quick_move(
            ...
            )
        {
            ...
            // marking the source slot; Foton's move works on a copy, so the
