...
// within the process.
unsafe impl foton_utils::DowncastType for FurnaceKind {
    const TYPE_KEY: foton_utils::DowncastTypeKey =
        foton_utils::DowncastTypeKey::new("foton:menu/furnace");
}
...
impl MenuKind for FurnaceKind {
    ...
    /// are the same object. Foton keeps the cooking state behind the block
