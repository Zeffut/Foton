...
use foton_registry::{potion_brewing, vanilla_items, vanilla_menu_types};
...
// within the process.
unsafe impl foton_utils::DowncastType for BrewingStandKind {
    const TYPE_KEY: foton_utils::DowncastTypeKey =
        foton_utils::DowncastTypeKey::new("foton:menu/brewing_stand");
}
...
impl MenuKind for BrewingStandKind {
    ...
    /// `ContainerData`; Foton republishes it here so the menu never has to take
