...
use foton_utils::{ChunkPos, Downcast as _, DowncastType, DowncastTypeKey, WorldAabb};
use glam::DVec3;
...
unsafe impl DowncastType for SingleItemSlot {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:test/slot/single_item");
}
...
fn draining_a_block_entity_slot_marks_its_chunk_dirty() {
    ...
    let pos = foton_utils::BlockPos::new(0, 64, 0);
