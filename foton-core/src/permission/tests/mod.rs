...
fn scoped_child_rules_override_parent_fallbacks() {
    ...
    assert!(!permissions.allows_scoped_key(&parent, &key("foton.command.fly")));
}

#[test]
fn expression_all_and_any_preserve_unset_state() {
    let permissions = PermissionSet::from_entries([
        PermissionEntry::allow(key("foton.allowed")),
        PermissionEntry::deny(key("foton.denied")),
    ]);
    let allowed = PermissionExpr::key(key("foton.allowed"));
    let denied = PermissionExpr::key(key("foton.denied"));
    let unset = PermissionExpr::key(key("foton.unset"));
...
fn source_and_group_priority_break_only_equal_specificity_ties() {
    ...
    let contextual = key("foton.contextual");
