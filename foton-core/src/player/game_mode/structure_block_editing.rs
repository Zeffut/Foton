...
//! server.
//!
//! The structure block's editor has four buttons, and one of them Foton cannot
...
use foton_utils::{BlockPos, Downcast as _, Identifier, translations};
...
impl Player {
    ...
    fn report_structure_action(
        ...
        let message = match update_type {
            ...
            // Foton cannot capture a structure yet, so this is always the
...
/// the codec; Foton keeps the string and falls back here so a bad editor entry
