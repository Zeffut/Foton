...
use foton_protocol::packets::game::EquipmentSlotItem;
...
use foton_registry::data_components::vanilla_components::{CAN_BREAK, EQUIPPABLE};
use foton_registry::data_components::{AdventureModePredicate, BlockPredicate};
use foton_registry::entity_data::EntityData;
use foton_registry::packets::play::C_REMOVE_ENTITIES;
use foton_registry::{
...
use foton_utils::types::{Difficulty, GameType, InteractionHand, UpdateFlags};
use foton_utils::{BlockPos, ChunkPos, Downcast as _, DowncastType, DowncastTypeKey, WorldAabb};
use glam::DVec3;
...
fn test_persistent_entity(
    entity_type: foton_utils::Identifier,
    uuid: [u8; 16],
) -> PersistentEntity {
    PersistentEntity {
        entity_type,
...
impl_test_menu_kind_downcast!(CountRemovals, "foton:test/menu/player/count_removals");
...
impl_test_menu_kind_downcast!(ReopenOnRemoved, "foton:test/menu/player/reopen_on_removed");
...
fn permission_state_replacement_is_versioned_and_keeps_both_rule_sets() {
    ...
    let overrides =
        PermissionSet::from_entries([PermissionEntry::deny(permission_key("foton.fly"))]);
    let effective =
        PermissionSet::from_entries([PermissionEntry::allow(permission_key("foton.build"))]);
    ...
    assert!(!state.overrides.allows_key(&permission_key("foton.fly")));
    assert!(state.effective.allows_key(&permission_key("foton.build")));
}
...
fn death_respawn_redetects_unchanged_kept_equipment() {
    ...
    // Foton resets the reused player's transient attributes.
    player.reset_state_for_death_respawn();
    assert_eq!(
        player
            .attributes()
...
/// alone. The bounds matter as much as the hits -- Foton keeps the worn
...
fn a_player_answers_for_each_of_its_own_slot_ranges() {
    ...
    *player.inventory_menu.lock().behavior_mut().carried_mut() = stick.clone();
    ...
    // Storage, and the slot just past it: Foton's player container runs on
