...
//! to flush map updates. Foton keeps the split, because the second pass must
...
impl Player {
    ...
    /// `EntityEquipment.tick` from `LivingEntity.tick`. Foton runs both in one
    ...
    /// inventory again, and Foton's mutex is not reentrant.
