...
impl Player {
    ...
    ///
    /// Foton gap: Vanilla first calls `WrittenBookContent.resolveForItem`, which
    /// expands selectors and scores inside a command-written book and rewrites
    /// the component as resolved. Foton has no command-source text resolution,
    ...
    pub fn handle_edit_book(&self, packet: SEditBook) {
        ...
        // resumes on the main thread. Foton has no filter service, so both
    ...
    /// reusable because Foton keeps one `Player` across world changes.
