...
use super::{
    ...
    execution::{
        ...
        CommandSuspensionOrder, FotonArgumentParser, FotonArgumentSuggestionContext,
        FotonArgumentType, FotonCommandContext, FotonCommandRuntime,
    },
...
pub struct CommandNode {
    inner: CommandNodeBuilder<InternalCommandSource, FotonCommandRuntime>,
}
...
pub struct CommandContext<'context> {
    inner: &'context FotonCommandContext<InternalCommandSource>,
}
...
impl CommandArgument {
    ...
    pub fn entity() -> Self {
        Self {
            inner: FotonArgumentType::entity(),
        }
    }
    ...
    pub fn entities() -> Self {
        Self {
            inner: FotonArgumentType::entities(),
        }
    }
    ...
    pub fn player() -> Self {
        Self {
            inner: FotonArgumentType::player(),
        }
    }
    ...
    pub fn players() -> Self {
        Self {
            inner: FotonArgumentType::players(),
        }
    }

    /// Creates a configured Foton domain argument.
    #[must_use]
    pub fn domain() -> Self {
        Self {
            inner: FotonArgumentType::domain(),
        }
    }
    ...
    {
        Self {
            inner: FotonArgumentType::new(parser),
        }
    }

    fn primitive(argument: ArgumentType) -> Self {
        Self {
            inner: FotonArgumentType::from(argument),
        }
    }
}
...
impl<P> FotonArgumentParser for P
where
    P: CommandArgumentParser,
{
    type Value = P::Value;
...
pub struct CommandSuggestionContext<'context> {
    inner: &'context dyn FotonArgumentSuggestionContext,
}
...
/// Cross-tick command work owned and cancelled by Foton's command scheduler.
