...
pub(super) fn as_operation() -> Builder {
    literal("as").then(argument("targets", FotonArgumentType::entities()).forks(
        EXECUTE_ROOT,
        |context| {
            Ok(context
...
pub(super) fn at_operation() -> Builder {
    literal("at").then(argument("targets", FotonArgumentType::entities()).forks(
        EXECUTE_ROOT,
        |context| {
            context
                .optional_entities("targets")?
...
pub(super) fn positioned_operation() -> Builder {
    literal("positioned")
        .then(
            argument("pos", FotonArgumentType::vec3(true)).redirects_with(
                EXECUTE_ROOT,
        ...
        .then(literal("over").then(
            argument("heightmap", FotonArgumentType::heightmap()).redirects_with(
                EXECUTE_ROOT,
                |context: &FotonCommandContext<CommandSource>| {
...
pub(super) fn rotated_operation() -> Builder {
    literal("rotated")
        .then(
            argument("rot", FotonArgumentType::rotation()).redirects_with(
                EXECUTE_ROOT,
                |context| {
...
pub(super) fn facing_operation() -> Builder {
    literal("facing")
        .then(
            literal("entity").then(argument("targets", FotonArgumentType::entities()).then(
                argument("anchor", FotonArgumentType::entity_anchor()).forks(
                    EXECUTE_ROOT,
                    |context| {
...
pub(super) fn align_operation() -> Builder {
    literal("align").then(
        argument("axes", FotonArgumentType::swizzle()).redirects_with(
            EXECUTE_ROOT,
            |context: &FotonCommandContext<CommandSource>| {
...
pub(super) fn anchored_operation() -> Builder {
    literal("anchored").then(
        argument("anchor", FotonArgumentType::entity_anchor()).redirects_with(
            EXECUTE_ROOT,
            |context: &FotonCommandContext<CommandSource>| {
                let anchor = context.entity_anchor("anchor")?;
...
pub(super) fn in_operation() -> Builder {
    literal("in").then(
        argument("dimension", FotonArgumentType::world()).redirects_with(
            EXECUTE_ROOT,
            |context: &FotonCommandContext<CommandSource>| {
                let world = context
...
pub(super) fn summon_operation() -> Builder {
    literal("summon").then(
        argument("entity", FotonArgumentType::summonable_entity()).redirects_with(
            EXECUTE_ROOT,
            |context| {
