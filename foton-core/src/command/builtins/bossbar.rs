        ...
        .then(
            literal("add").then(
                argument("id", FotonArgumentType::boss_bar_id())
                    .then(argument("name", FotonArgumentType::component()).executes(create_bar)),
            ),
        )
        .then(
            literal("remove")
                .then(argument("id", FotonArgumentType::boss_bar_id()).executes(remove_bar)),
        )
...
fn set_subcommands() -> CommandNodeBuilder<CommandSource, FotonCommandRuntime> {
    ...
    argument("id", FotonArgumentType::boss_bar_id())
        .then(
            literal("name")
                .then(argument("name", FotonArgumentType::component()).executes(set_name)),
        )
        .then(colors)
        .then(styles)
        .then(
        ...
        .then(
            literal("players")
                ...
                .then(
                    argument("targets", FotonArgumentType::players()).executes(|context| {
...
///
/// Vanilla reaches one server-wide collection. Foton keeps them per domain,
...
fn create_bar(context: &FotonCommandContext<CommandSource>) -> Result<i32, CommandSyntaxError> {
    let id = context.boss_bar_id("id")?.clone();
...
fn remove_bar(context: &FotonCommandContext<CommandSource>) -> Result<i32, CommandSyntaxError> {
...
fn list_bars(context: &FotonCommandContext<CommandSource>) -> Result<i32, CommandSyntaxError> {
...
fn get_value(context: &FotonCommandContext<CommandSource>) -> Result<i32, CommandSyntaxError> {
    let bar = boss_bar(context)?;
...
fn get_max(context: &FotonCommandContext<CommandSource>) -> Result<i32, CommandSyntaxError> {
    let bar = boss_bar(context)?;
...
fn get_visible(context: &FotonCommandContext<CommandSource>) -> Result<i32, CommandSyntaxError> {
    let bar = boss_bar(context)?;
...
fn get_players(context: &FotonCommandContext<CommandSource>) -> Result<i32, CommandSyntaxError> {
    let bar = boss_bar(context)?;
...
fn set_name(context: &FotonCommandContext<CommandSource>) -> Result<i32, CommandSyntaxError> {
    let bar = boss_bar(context)?;
    let name = context.text_component("name")?.clone();
...
fn set_value(context: &FotonCommandContext<CommandSource>) -> Result<i32, CommandSyntaxError> {
    let bar = boss_bar(context)?;
...
fn set_max(context: &FotonCommandContext<CommandSource>) -> Result<i32, CommandSyntaxError> {
    let bar = boss_bar(context)?;
...
fn set_visible(context: &FotonCommandContext<CommandSource>) -> Result<i32, CommandSyntaxError> {
    let bar = boss_bar(context)?;
