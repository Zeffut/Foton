//! Vanilla experience command plus Foton's existing clear extension.
...
fn command() -> CommandNodeBuilder<CommandSource, FotonCommandRuntime> {
    literal("experience")
        ...
        .then(
            literal("query").then(
                argument("target", FotonArgumentType::player())
        ...
        .then(
            literal("clear")
                ...
                .then(argument("target", FotonArgumentType::players()).executes(clear_targets)),
        )
}
...
fn experience_operation(
    name: &'static str,
    minimum: i32,
    points: fn(&FotonCommandContext<CommandSource>) -> Result<i32, CommandSyntaxError>,
    levels: fn(&FotonCommandContext<CommandSource>) -> Result<i32, CommandSyntaxError>,
) -> CommandNodeBuilder<CommandSource, FotonCommandRuntime> {
    literal(name).then(
        argument("target", FotonArgumentType::players()).then(
...
fn query_points(context: &FotonCommandContext<CommandSource>) -> Result<i32, CommandSyntaxError> {
    query_experience(context, ExperienceType::Points)
}

fn query_levels(context: &FotonCommandContext<CommandSource>) -> Result<i32, CommandSyntaxError> {
...
fn add_points(context: &FotonCommandContext<CommandSource>) -> Result<i32, CommandSyntaxError> {
    add_experience(context, ExperienceType::Points)
}

fn add_levels(context: &FotonCommandContext<CommandSource>) -> Result<i32, CommandSyntaxError> {
...
fn set_points(context: &FotonCommandContext<CommandSource>) -> Result<i32, CommandSyntaxError> {
    set_experience(context, ExperienceType::Points)
}

fn set_levels(context: &FotonCommandContext<CommandSource>) -> Result<i32, CommandSyntaxError> {
...
fn clear_source(context: &FotonCommandContext<CommandSource>) -> Result<i32, CommandSyntaxError> {
...
fn clear_targets(context: &FotonCommandContext<CommandSource>) -> Result<i32, CommandSyntaxError> {
    let players = context.players("target")?;
    for player in &players {
        player.experience.lock().clear();
    }
        ...
        for root_name in ["experience", "xp"] {
            ...
            assert_eq!(
                dispatcher
                    .node(add_amount)
                    .and_then(|node| node.argument_type()),
                Some(&FotonArgumentType::from(ArgumentType::integer(
                    i32::MIN,
                    i32::MAX
                )))
            );
            ...
            assert_eq!(
                dispatcher
                    .node(set_amount)
                    .and_then(|node| node.argument_type()),
                Some(&FotonArgumentType::from(ArgumentType::integer(0, i32::MAX)))
            );
            assert!(
                dispatcher
                    .node(set_amount)
