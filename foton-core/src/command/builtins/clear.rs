...
fn command() -> CommandNodeBuilder<CommandSource, FotonCommandRuntime> {
    literal("clear").executes(clear_self).then(
        argument("targets", FotonArgumentType::players())
            ...
            .then(
                argument("item", FotonArgumentType::item_predicate())
...
fn clear_matching(context: &FotonCommandContext<CommandSource>) -> Result<i32, CommandSyntaxError> {
...
mod tests {
    ...
    fn clear_graph_matches_vanilla_argument_shape() {
        ...
        assert_eq!(
            ...
            Some(&FotonArgumentType::item_predicate())
        );
        assert!(matches!(
