//! Foton permission administration under `/perms`.
...
use super::super::{
    ...
    execution::{
        ...
        CommandSource, CommandSuspensionOrder, FotonArgumentType, FotonCommandContext,
        FotonCommandRuntime, GameProfileArgument, argument, literal,
    },
...
fn user_command() -> CommandNodeBuilder<CommandSource, FotonCommandRuntime> {
    literal("user").then(
        argument("targets", FotonArgumentType::game_profile())
            .then(literal("info").executes_suspended(user_info))
            .then(
                literal("allow").then(
                    argument("permission", FotonArgumentType::permission_rule())
            ...
            .then(
                literal("unset").then(
                    argument("permission", FotonArgumentType::user_permission_rule())
            ...
            .then(
                literal("group")
                    .then(
                        literal("add").then(
                            argument("group", FotonArgumentType::permission_group(true))
...
fn user_metadata_command() -> CommandNodeBuilder<CommandSource, FotonCommandRuntime> {
    literal("metadata")
        .then(metadata_set_command(user_metadata_set))
        .then(
            literal("check").then(
                argument("metadata", FotonArgumentType::permission_metadata())
                    .executes_suspended(user_metadata_check),
            ),
        )
        .then(
            literal("unset").then(
                argument("metadata", FotonArgumentType::user_permission_metadata())
                    .executes_suspended(user_metadata_unset),
            ),
        )
}

fn group_command() -> CommandNodeBuilder<CommandSource, FotonCommandRuntime> {
    literal("group").then(
        argument("group", FotonArgumentType::permission_group(false))
            ...
            .then(
                literal("unset").then(
                    argument("permission", FotonArgumentType::group_permission_rule())
            ...
            .then(
                literal("inherit")
                    ...
                    .then(
                        literal("add").then(
                            argument("parent", FotonArgumentType::permission_group(true))
            ...
            .then(
                literal("metadata")
                    ...
                    .then(
                        literal("unset").then(
                            argument("metadata", FotonArgumentType::group_permission_metadata())
...
fn groups_command() -> CommandNodeBuilder<CommandSource, FotonCommandRuntime> {
    literal("groups")
...
fn metadata_set_command(
    executor: fn(
        &FotonCommandContext<CommandSource>,
...
fn require_permission_management(
    ...
    require_dynamic_permission(
        source,
        format!("foton.permission.manage.{}", permission.as_str()),
    )
}

fn require_group_management(source: &CommandSource, group: &str) -> Result<(), CommandSyntaxError> {
    require_dynamic_permission(source, format!("foton.permission.group.{group}"))
}
...
fn can_manage_group(source: &CommandSource, group: &str) -> bool {
    has_dynamic_permission(source, &format!("foton.permission.group.{group}"))
}
...
mod tests {
    ...
    use crate::command::{
        ...
        execution::{CommandSource, FotonCommandRuntime},
    };
    ...
    fn perms_exposes_the_management_surface_without_old_aliases() {
        ...
        assert!(!roots.iter().any(|root| {
            dispatcher
                .node(*root)
                .is_some_and(|node| matches!(node.name(), "fotonperms" | "sp"))
        }));
    ...
    fn perms_discovery_contains_static_admin_and_granular_command_permissions() {
        ...
        for expected in [
            ...
            METADATA_PERMISSION,
            "foton.command.perms.user.allow",
            "foton.command.perms.user.metadata.set",
            "foton.command.perms.group.inherit.add",
        ] {
