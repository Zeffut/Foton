...
mod tests {
    ...
    fn permission_edits_replace_the_exact_context_only() {
        ...
        let global = permission("foton.build");
        let survival = permission("foton.build{domain=survival}");
    ...
    fn permission_edits_replace_semantically_equal_context_orderings() {
        ...
        group
            .allow
            .push("foton.build{plugin:region=spawn,domain=survival}".to_owned());
        let expression = permission("foton.build{domain=survival,plugin:region=spawn}");
        ...
        assert_eq!(
            config.groups["default"].deny,
            ["foton.build{domain=survival,plugin:region=spawn}"]
        );
    }
