...
use foton_registry::vanilla_game_rules::SEND_COMMAND_FEEDBACK;
use foton_utils::{Identifier, translations, types::GameType};
use text_components::{TextComponent, translation::Translation};

use super::super::{
    brigadier::{CommandNodeBuilder, CommandSyntaxError},
    execution::{
        CommandPermissionSource, CommandSource, FotonArgumentType, FotonCommandContext,
...
fn command() -> CommandNodeBuilder<CommandSource, FotonCommandRuntime> {
    literal("gamemode").then(
        argument("gamemode", FotonArgumentType::game_mode())
            .executes(set_own_game_mode)
            .then(argument("target", FotonArgumentType::players()).executes(set_target_game_mode)),
    )
}
        ...
        assert!(matches!(
            ...
            Some(node)
                if node.is_executable()
                    && node.argument_type() == Some(&FotonArgumentType::game_mode())
        ));
