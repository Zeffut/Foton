...
#[cfg(test)]
use super::{brigadier::CommandDispatcher, execution::FotonCommandRuntime};

#[cfg(test)]
pub(crate) fn create_dispatcher()
-> Result<CommandDispatcher<CommandSource, FotonCommandRuntime>, CommandRegistrationError> {
...
mod tests {
    ...
    fn first_builtin_slice_has_the_expected_graph_shape() {
        ...
        let weather_names = weather_children
            ...
            .map(|child| {
                ...
                assert_eq!(duration.argument_type(), Some(&FotonArgumentType::time(1)));
                node.name()
            })
            .collect::<Vec<_>>();
    ...
    fn startup_extensions_merge_after_builtins_with_namespaced_collision_fallbacks() {
        ...
        let registration =
            ExtensionCommandRegistration::new(Identifier::new("foton_test", "stop"), || {
        ...
        assert!(names.contains(&"foton_test:stop"));
        assert!(
            registered
                .permissions
                .iter()
                .any(|permission| permission.as_str() == "foton_test.command.stop")
        );
    }

    #[test]
    fn invsee_discovers_only_foton_namespaced_permissions() {
        init_vanilla_registry();
        ...
        assert!(permissions.contains(&"foton.command.invsee"));
        assert!(permissions.contains(&"foton.command.invsee.modify"));
    ...
    fn execute_graph_uses_expected_redirects_and_argument_types() {
        ...
        for condition in ["if", "unless"] {
            for (path, expected_type) in [
                (
                    ...
                    FotonArgumentType::entities(),
                ),
                ([condition, "loaded", "pos"], FotonArgumentType::block_pos()),
            ] {
                let terminal = path
                    .iter()
            ...
            for position in [start, end, destination] {
                assert_eq!(
                    ...
                    Some(&FotonArgumentType::block_pos())
                );
            }
            ...
            for (provider, source_name, source_type) in [
                ("block", "sourcePos", FotonArgumentType::block_pos()),
                ("entity", "source", FotonArgumentType::entity()),
                ("storage", "source", FotonArgumentType::storage_key()),
            ] {
                ...
                assert!(node.is_executable());
                assert_eq!(node.redirect(), Some(execute));
                assert_eq!(node.argument_type(), Some(&FotonArgumentType::nbt_path()));
            }
            ...
            assert!(node.is_executable());
            assert_eq!(node.redirect(), Some(execute));
            assert_eq!(node.argument_type(), Some(&FotonArgumentType::world()));
            ...
            assert_eq!(
                dispatcher
                    .node(target)
                    .and_then(|node| node.argument_type()),
                Some(&FotonArgumentType::score_holder())
            );
            ...
            assert_eq!(
                dispatcher
                    ...
                    .and_then(|node| node.argument_type()),
                Some(&FotonArgumentType::objective())
            );
            for comparison in ["=", "<", "<=", ">", ">="] {
                ...
                assert!(node.is_executable());
                assert_eq!(node.redirect(), Some(execute));
                assert_eq!(node.argument_type(), Some(&FotonArgumentType::objective()));
            }
            ...
            assert_eq!(
                ...
                Some(&FotonArgumentType::int_range())
            );
        }
        ...
        for store_kind in ["result", "success"] {
            ...
            assert_eq!(
                dispatcher
                    .node(targets)
                    .and_then(|node| node.argument_type()),
                Some(&FotonArgumentType::score_holders())
            );
            ...
            for (provider, target_name, target_type) in [
                ("block", "targetPos", FotonArgumentType::block_pos()),
                ("storage", "target", FotonArgumentType::storage_key()),
            ] {
                ...
                assert_eq!(
                    ...
                    Some(&FotonArgumentType::nbt_path())
                );
                for data_type in ["int", "float", "short", "long", "double", "byte"] {
                    ...
                    assert_eq!(
                        ...
                        Some(&FotonArgumentType::from(ArgumentType::double(
                            f64::MIN,
                            f64::MAX,
                        )))
                    );
        ...
        let argument_types: &[(&[&str], FotonArgumentType)] = &[
            (&["as", "targets"], FotonArgumentType::entities()),
            (&["at", "targets"], FotonArgumentType::entities()),
            (&["positioned", "pos"], FotonArgumentType::vec3(true)),
            (
                &["positioned", "over", "heightmap"],
                FotonArgumentType::heightmap(),
            ),
            (&["rotated", "rot"], FotonArgumentType::rotation()),
            (&["facing", "pos"], FotonArgumentType::vec3(true)),
            (
                &["facing", "entity", "targets", "anchor"],
                FotonArgumentType::entity_anchor(),
            ),
            (&["align", "axes"], FotonArgumentType::swizzle()),
            (&["anchored", "anchor"], FotonArgumentType::entity_anchor()),
            (
                &["summon", "entity"],
                FotonArgumentType::summonable_entity(),
            ),
        ];
