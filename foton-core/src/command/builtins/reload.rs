...
//! resource. Functions are the only one Foton loads from a datapack today --
//! loot tables, recipes and advancements are compiled in by the build scripts --
//! so reloading them is the whole of a reload here.
//!
//! Vanilla blocks its server thread on the reload. A Foton tick may not wait,
...
use super::super::{
    ...
    execution::{
        ...
        CommandSuspensionOrder, FotonCommandContext, FotonCommandRuntime, literal,
    },
