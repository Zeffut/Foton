...
//!
//! Vanilla applies clock mutations server-wide. Foton intentionally applies
...
use foton_registry::{timeline::TimelineRef, world_clock::WorldClockRef};
...
fn command() -> CommandNodeBuilder<CommandSource, FotonCommandRuntime> {
    ...
    command.then(literal("of").then(add_clock_nodes(
        argument(CLOCK_ARGUMENT, FotonArgumentType::world_clock()),
...
fn add_clock_nodes(
    node: CommandNodeBuilder<CommandSource, FotonCommandRuntime>,
    ...
    let mut query = literal("query")
        ...
        .then(
            argument(
                "timeline",
                FotonArgumentType::timeline(selection.argument_name()),
            )
    ...
    node.then(
        literal("set")
            .then(
                argument("time", FotonArgumentType::time(0))
                    .executes(move |context| set_total_ticks(context, selection)),
            )
            .then(
                argument(
                    "timemarker",
                    FotonArgumentType::time_marker(selection.argument_name()),
                )
    ...
    .then(
        literal("add").then(
            argument("time", FotonArgumentType::time(i32::MIN))
...
mod tests {
    ...
    use foton_registry::{init_vanilla_registry, vanilla_timelines, vanilla_world_clocks};
    use foton_utils::{Identifier, translations};
    ...
    fn time_graph_matches_the_26_2_clock_shape() {
        ...
        assert_eq!(
            ...
            Some(&FotonArgumentType::time_marker(None))
        );
        ...
        assert_eq!(
            dispatcher
                .node(rate_value)
                .and_then(|node| node.argument_type()),
            Some(&FotonArgumentType::from(ArgumentType::float(
                1.0E-5, 1_000.0
            )))
        );
        ...
        assert_eq!(
            ...
            Some(&FotonArgumentType::world_clock())
        );
        ...
        assert_eq!(
            dispatcher
                .node(of_marker)
                .and_then(|node| node.argument_type()),
            Some(&FotonArgumentType::time_marker(Some("clock")))
        );
    }
