...
use super::super::{
    ...
    execution::{
        ...
        ExecutionCommandSource, ExecutionControl, FotonArgumentType, FotonCommandRuntime,
        FotonContextChain, FunctionEntries, FunctionOrTag, argument, literal,
    },
...
fn command() -> CommandNodeBuilder<CommandSource, FotonCommandRuntime> {
    ...
    literal("function").then(
        argument("name", FotonArgumentType::function())
            .executes_custom(RunFunction)
            .then(
                argument("arguments", FotonArgumentType::nbt_compound_tag())
                    .executes_custom(RunFunction),
            ),
    )
}
...
fn queue_functions(
    ...
    // caps the permission level at gamemaster; Foton has no level ladder to cap,
...
mod tests {
    ...
    fn function_graph_takes_a_name_and_optional_macro_arguments() {
        ...
        assert_eq!(name.argument_type(), Some(&FotonArgumentType::function()));
