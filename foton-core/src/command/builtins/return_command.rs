...
use super::super::{
    ...
    execution::{
        ...
        ExecutionCommandSource, ExecutionControl, FotonCommandRuntime, FotonContextChain, argument,
        literal,
    },
...
fn command(dispatcher_root: NodeId) -> CommandNodeBuilder<CommandSource, FotonCommandRuntime> {
    literal("return")
        .then(
...
impl CustomCommandExecutor<CommandSource> for ReturnFail {
    fn run(
        &self,
        source: Arc<CommandSource>,
        _chain: &FotonContextChain<CommandSource>,
        _modifiers: ChainModifiers,
        control: &mut ExecutionControl<'_, CommandSource>,
    ) {
...
mod tests {
    ...
    use crate::command::{brigadier::ArgumentType, execution::FotonArgumentType};
