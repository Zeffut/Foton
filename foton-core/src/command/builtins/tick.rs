    ...
    literal("tick")
        ...
        .then(
            literal("step")
                ...
                .then(
                    argument("time", FotonArgumentType::time(1)).executes(|context| {
...
fn query_tick(context: &FotonCommandContext<CommandSource>) -> Result<i32, CommandSyntaxError> {
...
fn set_tick_rate(context: &FotonCommandContext<CommandSource>) -> Result<i32, CommandSyntaxError> {
...
fn stop_step(context: &FotonCommandContext<CommandSource>) -> Result<i32, CommandSyntaxError> {
    let stopped = context
        .source()
        .server()
        .tick_rate_manager
...
fn stop_sprint(context: &FotonCommandContext<CommandSource>) -> Result<i32, CommandSyntaxError> {
    let report = context
        .source()
        .server()
        .tick_rate_manager
...
mod tests {
    ...
    fn tick_graph_matches_the_target_shape_and_permissions() {
        ...
        assert_eq!(
            ...
            Some(&FotonArgumentType::from(ArgumentType::float(1.0, 10_000.0)))
        );
