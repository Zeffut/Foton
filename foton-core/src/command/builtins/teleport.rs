...
use super::super::{
    ...
    execution::{
        CommandSource, Coordinates, FotonArgumentType, FotonCommandContext, FotonCommandRuntime,
        argument, literal,
    },
...
fn command() -> CommandNodeBuilder<CommandSource, FotonCommandRuntime> {
    literal("teleport")
        .then(
            argument("location", FotonArgumentType::vec3(true))
                .executes(teleport_source_to_position),
        )
        .then(
            argument("destination", FotonArgumentType::entity())
...
fn target_position_branch() -> CommandNodeBuilder<CommandSource, FotonCommandRuntime> {
    argument("location", FotonArgumentType::vec3(true))
        .executes(teleport_targets_to_position)
        .then(
            argument("rotation", FotonArgumentType::rotation())
        ...
        .then(
            literal("facing")
                .then(
                    literal("entity").then(
                        argument("facingEntity", FotonArgumentType::entity())
                            .executes(teleport_targets_facing_entity_feet)
                            .then(
                                argument("facingAnchor", FotonArgumentType::entity_anchor())
                                    .executes(teleport_targets_facing_entity_anchor),
                            ),
                    ),
                )
                .then(
                    argument("facingLocation", FotonArgumentType::vec3(true))
...
fn ensure_same_domain(source_domain: &str, target_domain: &str) -> Result<(), CommandSyntaxError> {
    ...
    Err(CommandSyntaxError::dynamic(
        "Entities cannot be teleported across Foton domains",
    ))
}
    ...
    fn teleport_graph_matches_vanilla_target_and_facing_shapes() {
        ...
        for root_name in ["teleport", "tp"] {
            ...
            assert!(matches!(
                ...
                Some(node)
                    if node.is_executable()
                        && node.argument_type() == Some(&FotonArgumentType::vec3(true))
            ));
            ...
            assert!(matches!(
                ...
                Some(node)
                    if node.is_executable()
                        && node.argument_type() == Some(&FotonArgumentType::entity())
            ));
            ...
            assert!(matches!(
                ...
                Some(node)
                    if node.is_executable()
                        && node.argument_type() == Some(&FotonArgumentType::rotation())
            ));
            ...
            assert!(matches!(
                ...
                Some(node)
                    if node.is_executable()
                        && node.argument_type() == Some(&FotonArgumentType::entity_anchor())
            ));
