...
pub(super) fn registration() -> CommandRegistration<CommandSource> {
    CommandRegistration::new(Identifier::from_foton("domain"), |_| command())
}

fn command() -> CommandNodeBuilder<CommandSource, FotonCommandRuntime> {
    literal("domain")
        .executes(|ctx: &FotonCommandContext<CommandSource>| {
        ...
        .then(argument("world", FotonArgumentType::world()).executes(switch_world))
}

fn switch_world(context: &FotonCommandContext<CommandSource>) -> Result<i32, CommandSyntaxError> {
...
// within the process.
unsafe impl foton_utils::DowncastType for DomainMenuKind {
    const TYPE_KEY: foton_utils::DowncastTypeKey =
        foton_utils::DowncastTypeKey::new("foton:menu/domain");
}
