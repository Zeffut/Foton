...
use foton_utils::translations::{COMMANDS_DAMAGE_INVULNERABLE, COMMANDS_DAMAGE_SUCCESS};
    ...
    literal("damage").then(
        argument("target", FotonArgumentType::entity()).then(
            argument("amount", ArgumentType::float(0.0, f32::MAX))
                ...
                .then(
                    argument("damageType", FotonArgumentType::damage_type())
                        .executes(|context| damage(context, typed_source(context)?))
                        .then(literal("at").then(
                            argument("location", FotonArgumentType::vec3(true)).executes(
                                |context| {
                                    let position =
                        ...
                        .then(
                            literal("by").then(
                                argument("entity", FotonArgumentType::entity())
                                    .executes(|context| {
                                    ...
                                    .then(literal("from").then(
                                        argument("cause", FotonArgumentType::entity()).executes(
                                            |context| {
                                                let entity = context.entity("entity")?.id();
