...
pub(super) fn registration() -> CommandRegistration<CommandSource> {
    CommandRegistration::new(Identifier::from_foton("fly"), |_| command())
}
...
fn speed_argument() -> CommandNodeBuilder<CommandSource, FotonCommandRuntime> {
...
fn required_speed(context: &FotonCommandContext<CommandSource>) -> Result<f32, CommandSyntaxError> {
    context.float("speed")
}
...
mod tests {
    ...
    fn fly_graph_uses_explicit_target_and_bounded_speed_branches() {
        ...
        assert!(matches!(
            ...
            Some(node)
                if node.is_executable()
                    && node.argument_type() == Some(&FotonArgumentType::players())
        ));
        let value = child(&dispatcher, targets, "value");
        assert_eq!(
            dispatcher.node(value).and_then(|node| node.argument_type()),
            Some(&FotonArgumentType::from(ArgumentType::bool()))
        );
        ...
        let expected = FotonArgumentType::from(ArgumentType::float(0.0, MAX_FLY_SPEED_MULTIPLIER));
