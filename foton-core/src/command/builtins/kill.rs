...
fn command() -> CommandNodeBuilder<CommandSource, FotonCommandRuntime> {
    literal("kill")
        .executes(kill_self)
        .then(argument("targets", FotonArgumentType::entities()).executes(kill_targets))
}

fn kill_self(context: &FotonCommandContext<CommandSource>) -> Result<i32, CommandSyntaxError> {
...
fn kill_targets(context: &FotonCommandContext<CommandSource>) -> Result<i32, CommandSyntaxError> {
        ...
        assert!(matches!(
            ...
            Some(node)
                if node.is_executable()
                    && node.argument_type() == Some(&FotonArgumentType::entities())
        ));
    }
}
