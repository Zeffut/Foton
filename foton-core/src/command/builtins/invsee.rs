...
use super::super::{
    ...
    execution::{
        CommandPermissionSource, CommandSource, FotonArgumentType, FotonCommandRuntime, argument,
        literal,
    },
...
const INVSEE_PERMISSION: &str = "foton.command.invsee";
const MODIFY_PERMISSION: &str = "foton.command.invsee.modify";

pub(super) fn registration() -> Result<CommandRegistration<CommandSource>, CommandRegistrationError>
{
    let id = Identifier::from_foton("invsee");
...
fn command(
    ...
    literal("invsee").then(
        argument("target", FotonArgumentType::player()).executes(move |ctx| {
...
fn ensure_same_domain(source: &Player, target: &Player) -> Result<(), CommandSyntaxError> {
    ...
    Err(CommandSyntaxError::dynamic(
        "Invsee cannot open inventories across Foton domains",
    ))
}
...
// within the process.
unsafe impl foton_utils::DowncastType for InvseeMenuKind {
    const TYPE_KEY: foton_utils::DowncastTypeKey =
        foton_utils::DowncastTypeKey::new("foton:menu/invsee");
}
