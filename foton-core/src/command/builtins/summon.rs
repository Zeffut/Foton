...
use foton_utils::{BlockPos, Identifier, translations, types::Difficulty};
use glam::DVec3;
...
fn command() -> CommandNodeBuilder<CommandSource, FotonCommandRuntime> {
    literal("summon").then(
        argument("entity", FotonArgumentType::summonable_entity())
            .executes(|context| {
                summon_entity(
                    context,
            ...
            .then(
                argument("pos", FotonArgumentType::vec3(true))
                    .executes(|context| {
                    ...
                    .then(
                        argument("nbt", FotonArgumentType::nbt_compound_tag()).executes(
                            |context| {
                                let position = context.coordinates("pos")?;
...
fn load_entity_tree(
    ...
    // The `summonable_entity` argument already refuses a type Foton has no
...
mod tests {
    ...
    fn summon_graph_uses_typed_entity_and_deferred_position_arguments() {
        ...
        assert_eq!(
            ...
            Some(&FotonArgumentType::summonable_entity())
        );
        assert!(matches!(
        ...
        assert_eq!(
            dispatcher
                .node(position)
                .and_then(|node| node.argument_type()),
            Some(&FotonArgumentType::vec3(true))
        );
        assert!(matches!(
        ...
        // would be a syntax Foton invented and a client would not suggest.
        let nbt = child(&dispatcher, position, "nbt");
        assert_eq!(
            dispatcher.node(nbt).and_then(|node| node.argument_type()),
            Some(&FotonArgumentType::nbt_compound_tag())
        );
        assert!(matches!(
            dispatcher.node(nbt),
            Some(node) if node.is_executable()
