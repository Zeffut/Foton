...
use foton_utils::{Identifier, nbt::parse_snbt_argument};
...
fn registered_item_predicate_matches(
    ...
    template: &foton_registry::ItemStackTemplate,
) -> bool {
    predicate
        .items()
...
fn parse_attribute_holder(value: &str) -> Option<Vec<AttributeRef>> {
    if value.starts_with('#') {
        // TODO: Support attribute tags once Foton's attribute registry stores them.
        return None;
    }
    let key = parse_identifier_with_default_namespace(value)?;
    REGISTRY
