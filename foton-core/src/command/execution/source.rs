...
/// Source behavior required by the Foton command scheduler.
...
/// Permission lookup required while constructing and traversing Foton command trees.
...
/// contextual Foton permissions apply. Player permissions come from the
...
impl CommandAuthorizationContext {
    fn for_player(world: foton_utils::Identifier, permissions: PermissionSet) -> Self {
    ...
    fn for_command_block(world: foton_utils::Identifier, permissions: PermissionSet) -> Self {
        Self {
            permission_context: PermissionContext::for_world(world),
            player_permissions: Some(permissions),
    ...
    fn unrestricted(world: foton_utils::Identifier) -> Self {
        Self {
            permission_context: PermissionContext::for_world(world),
    ...
    pub(crate) fn with_world(&self, world: Arc<World>) -> Result<Self, CommandSyntaxError> {
        if !self.allows_execution_domain(world.domain()) {
            return Err(CommandSyntaxError::dynamic(
                "Players cannot execute commands across Foton domains",
            ));
        }
        let mut source = self.clone();
...
impl CommandArgumentSource for CommandSource {
    ...
    fn user_permission_metadata_suggestions(&self, targets: &GameProfileArgument) -> Vec<String> {
        if !has_permission_key(self, "foton.permission.metadata") {
            return Vec::new();
        }
        let mut suggestions = BTreeSet::new();
        for uuid in profile_argument_uuids(self, targets) {
    ...
    fn group_permission_metadata_suggestions(&self, group: &str) -> Vec<String> {
        if !has_permission_key(self, "foton.permission.metadata") || !can_manage_group(self, group)
        {
            return Vec::new();
        }
        self.server
...
fn can_manage_group(source: &CommandSource, group: &str) -> bool {
    has_permission_key(source, &format!("foton.permission.group.{group}"))
}
...
mod tests {
    ...
    use foton_registry::{entity_type::EntityTypeRef, vanilla_entities};
