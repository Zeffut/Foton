...
use super::{
    CommandResultCallback, ExecutionCommandSource, FotonContextChain, FotonExecutor, FotonModifier,
};
    ...
    S: ExecutionCommandSource,
{
    fn execute(self: Box<Self>, context: &mut CommandExecutionContext<S>, frame: Frame) {
        ...
        match executor {
            FotonExecutor::Standard(executor) => {
                complete_command_result(source.as_ref(), modifiers, executor(&command_context));
            }
            FotonExecutor::Suspended(executor) => match executor(&command_context) {
...
where
    S: ExecutionCommandSource,
{
    pub(crate) const fn new(chain: FotonContextChain<S>) -> Self {
        Self { chain }
    }
}
