...
/// Typed parser contract erased by [`FotonArgumentType`].
pub(crate) trait FotonArgumentParser:
    DowncastType + fmt::Debug + PartialEq + Send + Sync + 'static
{
    ...
    fn list_suggestions(
        &self,
        _context: &dyn FotonArgumentSuggestionContext,
...
trait ErasedFotonArgumentParser: ErasedType + fmt::Debug + Send + Sync {
    fn parse_erased(
        &self,
        reader: &mut StringReader<'_>,
        source: &dyn CommandArgumentSource,
    ) -> Result<FotonArgumentValue, CommandSyntaxError>;

    fn list_suggestions_erased(
        &self,
        context: &dyn FotonArgumentSuggestionContext,
        builder: &mut SuggestionsBuilder<'_>,
    );

    fn protocol_argument_erased(&self) -> (ProtocolArgumentType, Option<ProtocolSuggestionType>);

    fn equals_erased(&self, other: &dyn ErasedFotonArgumentParser) -> bool;
}

impl<P> ErasedFotonArgumentParser for P
where
    P: FotonArgumentParser,
{
    fn parse_erased(
        &self,
        reader: &mut StringReader<'_>,
        ...
        self.parse(reader, source).map(FotonArgumentValue::new)
    }
...
/// An extensible, keyed parser stored by Foton's command runtime.
#[derive(Clone)]
pub(crate) struct FotonArgumentType(Arc<dyn ErasedFotonArgumentParser>);

impl FotonArgumentType {
    /// Erases a concrete parser while retaining its deterministic type key.
    pub(crate) fn new(parser: impl FotonArgumentParser) -> Self {
        Self(Arc::new(parser))
    }
...
impl fmt::Debug for FotonArgumentType {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("FotonArgumentType")
...
impl PartialEq for FotonArgumentType {
    fn eq(&self, other: &Self) -> bool {
        self.0.equals_erased(other.0.as_ref())
    }
}

impl From<ArgumentType> for FotonArgumentType {
    fn from(argument: ArgumentType) -> Self {
        Self::new(PrimitiveParser(argument))
    }
}

trait ErasedFotonArgumentValue: ErasedType + fmt::Debug + Send + Sync {}

impl<T> ErasedFotonArgumentValue for T where T: DowncastType + fmt::Debug + Send + Sync {}

/// A keyed parsed value retained by Foton's command runtime.
#[derive(Clone)]
pub(crate) struct FotonArgumentValue(Arc<dyn ErasedFotonArgumentValue>);

impl FotonArgumentValue {
...
impl fmt::Debug for FotonArgumentValue {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("FotonArgumentValue")
            .field("type_key", &self.0.downcast_type_key())
            .field("value", &self.0)
            .finish()
    }
...
impl ContainsPrimitiveArgumentValue for FotonArgumentValue {
...
/// Suggestion context exposed to erased Foton argument parsers.
pub(crate) trait FotonArgumentSuggestionContext {
    fn source(&self) -> &dyn CommandArgumentSource;

    fn argument(&self, name: &str) -> Result<&FotonArgumentValue, CommandSyntaxError>;
}

impl<S> FotonArgumentSuggestionContext for ArgumentSuggestionContext<'_, S, FotonArgumentValue>
where
...
impl<S> CommandArgumentParser<S> for FotonArgumentType
where
    S: CommandArgumentSource,
{
    type Value = FotonArgumentValue;
...
macro_rules! impl_downcast_type {
    ($type:ty, $key:literal) => {
        // SAFETY: This Foton-owned key uniquely identifies the concrete type in the process.
...
impl_downcast_type!(PrimitiveArgumentValue, "foton:command/value/primitive");
impl_downcast_type!(Coordinates, "foton:command/value/coordinates");
impl_downcast_type!(EntityAnchor, "foton:command/value/entity_anchor");
impl_downcast_type!(CoordinateAxes, "foton:command/value/swizzle");
impl_downcast_type!(HeightmapType, "foton:command/value/heightmap");
impl_downcast_type!(EntitySelector, "foton:command/value/entity_selector");
impl_downcast_type!(ScoreHolderArgument, "foton:command/value/score_holder");
impl_downcast_type!(IntRange, "foton:command/value/int_range");
impl_downcast_type!(BiomeOrTag, "foton:command/value/biome_or_tag");
impl_downcast_type!(
    StructureOrTagKey,
    "foton:command/value/structure_or_tag_key"
);
impl_downcast_type!(BlockPredicate, "foton:command/value/block_predicate");
...
argument_value_wrapper!(
    ...
    "foton:command/value/entity_type"
);
argument_value_wrapper!(
    EnchantmentValue(EnchantmentRef),
    "foton:command/value/enchantment"
);
argument_value_wrapper!(
...
argument_value_wrapper!(
    ...
    "foton:command/value/identifier"
);
argument_value_wrapper!(
    WorldClockValue(WorldClockRef),
    "foton:command/value/world_clock"
);
argument_value_wrapper!(TimelineValue(TimelineRef), "foton:command/value/timeline");
...
macro_rules! unit_argument_parser {
    (
        ...
        impl FotonArgumentParser for $parser {
            type Value = $value;
...
impl_downcast_type!(PrimitiveParser, "foton:command/parser/primitive");

impl FotonArgumentParser for PrimitiveParser {
...
impl_downcast_type!(TimeParser, "foton:command/parser/time");

impl FotonArgumentParser for TimeParser {
...
unit_argument_parser!(
    BlockPosParser,
    "foton:command/parser/block_pos",
    Coordinates,
    parse | reader,
...
impl_downcast_type!(Vec3Parser, "foton:command/parser/vec3");

impl FotonArgumentParser for Vec3Parser {
...
unit_argument_parser!(
    HeightmapParser,
    "foton:command/parser/heightmap",
    HeightmapType,
    parse | reader,
...
unit_argument_parser!(
    EntityAnchorParser,
    "foton:command/parser/entity_anchor",
    EntityAnchor,
    parse | reader,
...
impl_downcast_type!(EntityParser, "foton:command/parser/entity");

impl FotonArgumentParser for EntityParser {
...
impl_downcast_type!(ScoreHolderParser, "foton:command/parser/score_holder");

impl FotonArgumentParser for ScoreHolderParser {
...
unit_argument_parser!(
    ObjectiveParser,
    "foton:command/parser/objective",
    ObjectiveValue,
    parse | reader,
...
unit_argument_parser!(
    IntRangeParser,
    "foton:command/parser/int_range",
    IntRange,
    parse | reader,
...
unit_argument_parser!(
    BiomeOrTagParser,
    "foton:command/parser/biome_or_tag",
    BiomeOrTag,
    parse | reader,
...
unit_argument_parser!(
    ...
    "foton:command/parser/structure_or_tag_key",
    StructureOrTagKey,
    parse | reader,
...
unit_argument_parser!(
    ...
    "foton:command/parser/block_predicate",
    BlockPredicate,
    parse | reader,
...
unit_argument_parser!(
    GameModeParser,
    "foton:command/parser/game_mode",
    GameModeValue,
    parse | reader,
...
unit_argument_parser!(
    ...
    "foton:command/parser/summonable_entity",
    EntityTypeValue,
    parse | reader,
...
unit_argument_parser!(
    EnchantmentParser,
    "foton:command/parser/enchantment",
    EnchantmentValue,
    parse | reader,
    _source | {
...
unit_argument_parser!(
    ItemStackParser,
    "foton:command/parser/item_stack",
    ItemStackValue,
    parse | reader,
...
unit_argument_parser!(
    ItemPredicateParser,
    "foton:command/parser/item_predicate",
    ItemPredicate,
    parse | reader,
...
unit_argument_parser!(
    BossBarIdParser,
    "foton:command/parser/boss_bar_id",
    IdentifierValue,
    parse | reader,
...
unit_argument_parser!(
    ItemSlotsParser,
    "foton:command/parser/item_slots",
    ItemSlotsValue,
    parse | reader,
...
unit_argument_parser!(
    NbtCompoundParser,
    "foton:command/parser/nbt_compound",
    NbtCompoundValue,
    parse | reader,
...
unit_argument_parser!(
    WorldClockParser,
    "foton:command/parser/world_clock",
    WorldClockValue,
    parse | reader,
    _source | {
        let key = parse_identifier(reader)?;
...
impl_downcast_type!(TimelineParser, "foton:command/parser/timeline");

impl FotonArgumentParser for TimelineParser {
...
impl_downcast_type!(TimeMarkerParser, "foton:command/parser/time_marker");

impl FotonArgumentParser for TimeMarkerParser {
