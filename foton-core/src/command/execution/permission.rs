...
use super::{
    ...
    argument::{FotonArgumentParser, FotonArgumentSuggestionContext},
};

// SAFETY: This Foton-owned key uniquely identifies the concrete parsed value.
unsafe impl DowncastType for PermissionRuleExpression {
    const TYPE_KEY: DowncastTypeKey =
        DowncastTypeKey::new("foton:command/value/permission_rule_expression");
}

// SAFETY: This Foton-owned key uniquely identifies the concrete parsed value.
unsafe impl DowncastType for PermissionMetadataExpression {
    const TYPE_KEY: DowncastTypeKey =
        DowncastTypeKey::new("foton:command/value/permission_metadata_expression");
}
...
unsafe impl DowncastType for PermissionGroupName {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:command/value/permission_group");
}
...
// SAFETY: This Foton-owned key uniquely identifies the concrete parser.
unsafe impl DowncastType for PermissionRuleParser {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:command/parser/permission_rule");
}

impl FotonArgumentParser for PermissionRuleParser {
...
unsafe impl DowncastType for PermissionMetadataParser {
    const TYPE_KEY: DowncastTypeKey =
        DowncastTypeKey::new("foton:command/parser/permission_metadata");
}

impl FotonArgumentParser for PermissionMetadataParser {
...
unsafe impl DowncastType for PermissionGroupParser {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:command/parser/permission_group");
}

impl FotonArgumentParser for PermissionGroupParser {
...
const fn permission_expression_argument() -> (ProtocolArgumentType, Option<ProtocolSuggestionType>)
{
    ...
    // lets the client cover Foton's full no-whitespace expression syntax.
    (
