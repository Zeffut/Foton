...
fn dispatcher(minimum: i32) -> TestDispatcher {
    ...
    let command = literal("duration").then(
        argument("value", FotonArgumentType::time(minimum)).executes(|context| {
...
fn entity_selector_argument_is_retained_for_deferred_resolution() {
    ...
    let dispatcher = resource_dispatcher(FotonArgumentType::players());
...
fn world_clock_argument_resolves_default_and_explicit_namespaces() {
    init_vanilla_registry();
    let dispatcher = resource_dispatcher(FotonArgumentType::world_clock());
...
fn time_marker_argument_retains_default_namespace_identifier() {
    init_vanilla_registry();
    let dispatcher = resource_dispatcher(FotonArgumentType::time_marker(None));
...
fn timeline_suggestions_use_the_preceding_clock_argument() {
    ...
    let mut dispatcher = TestDispatcher::new();
    let command =
        literal("timeline").then(argument("clock", FotonArgumentType::world_clock()).then(
            argument("value", FotonArgumentType::timeline(Some("clock"))).executes(|_| Ok(1)),
        ));
    assert!(dispatcher.register(command).is_ok());
