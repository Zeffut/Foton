...
unsafe impl DowncastType for ExtensionParser {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:test/command/parser/extension");
}
...
unsafe impl DowncastType for ExtensionValue {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:test/command/value/extension");
}
...
unsafe impl DowncastType for UnrelatedValue {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:test/command/value/unrelated");
}

impl FotonArgumentParser for ExtensionParser {
...
fn keyed_argument_erasure_accepts_new_parser_and_value_types() {
    let argument_type = FotonArgumentType::new(ExtensionParser);
...
fn keyed_parser_equality_includes_concrete_configuration() {
    let one_tick = FotonArgumentType::time(1);
    let two_ticks = FotonArgumentType::time(2);

    assert_eq!(one_tick.parser_type_key(), two_ticks.parser_type_key());
    assert_eq!(one_tick, FotonArgumentType::time(1));
    assert_ne!(one_tick, two_ticks);
    assert_ne!(
        one_tick.parser_type_key(),
        FotonArgumentType::block_pos().parser_type_key()
    );
}

#[test]
fn component_argument_parses_vanilla_snbt_forms() {
    let dispatcher = resource_dispatcher(FotonArgumentType::component());
...
fn permission_arguments_parse_contexts_and_suggest_discovered_values() {
    let dispatcher = resource_dispatcher(FotonArgumentType::permission_rule());
    let parse = dispatcher.parse(
        "resource foton.build{domain=alpha,plugin:region=spawn}",
        TestSource::new(),
    );
    let Ok(chain) = dispatcher.context_chain(parse) else {
    ...
    assert_eq!(
        chain
            ...
            .as_deref(),
        Ok("foton.build{domain=alpha,plugin:region=spawn}")
    );

    let parse = dispatcher.parse("resource foton.build{plugin:region=s", TestSource::new());
    ...
    assert!(
        suggestions
            .list()
            .iter()
            .any(|suggestion| { suggestion.text() == "foton.build{plugin:region=spawn}" })
    );

    let parse = dispatcher.parse("resource foton.build{world=alpha:", TestSource::new());
    let Ok(suggestions) = dispatcher.completion_suggestions(&parse) else {
    ...
    assert_eq!(
        suggestions
            ...
            .collect::<Vec<_>>(),
        [
            "foton.build{world=alpha:arena}",
            "foton.build{world=alpha:overworld}"
        ]
    );
}
...
fn permission_group_argument_can_require_a_configured_group() {
    let dispatcher = resource_dispatcher(FotonArgumentType::permission_group(true));
    assert!(
        dispatcher
    ...
    assert!(
        dispatcher
            .register(literal("user").then(
                argument("targets", FotonArgumentType::game_profile()).then(argument(
                    "permission",
                    FotonArgumentType::user_permission_rule(),
                )),
            ))
            .is_ok()
    );
    assert!(
        dispatcher
            .register(literal("group").then(
                argument("group", FotonArgumentType::permission_group(true)).then(argument(
                    "permission",
                    FotonArgumentType::group_permission_rule(),
                )),
            ))
            .is_ok()
    );
...
fn component_argument_preserves_list_siblings_and_following_nodes() {
    ...
    let command = literal("component").then(
        argument("value", FotonArgumentType::component()).then(literal("done").executes(|_| Ok(1))),
    );
