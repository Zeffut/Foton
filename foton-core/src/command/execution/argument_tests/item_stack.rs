...
fn item_stack_argument_parses_supported_components_and_registered_removals() {
    ...
    let dispatcher = resource_dispatcher(FotonArgumentType::item_stack());
...
fn item_stack_argument_parses_identifier_components() {
    ...
    let dispatcher = resource_dispatcher(FotonArgumentType::item_stack());
    let parse = dispatcher.parse(
        "resource stone[item_model='stone',tooltip_style='foton:tooltip',note_block_sound='minecraft:block.note_block.harp']",
        TestSource::new(),
    );
    let Ok(chain) = dispatcher.context_chain(parse) else {
    ...
    assert_eq!(
        ...
        Some(&Identifier::new_static("foton", "tooltip"))
    );
    assert_eq!(
...
fn item_stack_argument_parses_custom_data_codecs() {
    ...
    let dispatcher = resource_dispatcher(FotonArgumentType::item_stack());
    let parse = dispatcher.parse(
        "resource stone[custom_data={value:7,nested:{name:'foton'}},bucket_entity_data='{Health:4.0f}']",
        TestSource::new(),
    );
    let Ok(chain) = dispatcher.context_chain(parse) else {
...
fn item_stack_argument_parses_custom_model_data_and_enchantability() {
    ...
    let dispatcher = resource_dispatcher(FotonArgumentType::item_stack());
    let parse = dispatcher.parse(
        "resource golden_sword[custom_model_data={floats:[1.5f],flags:[1b,0b],strings:['foton'],colors:[[1f,0.5f,0f]]},enchantable={value:5}]",
        TestSource::new(),
    );
    let Ok(chain) = dispatcher.context_chain(parse) else {
    ...
    assert_eq!(model_data.get_string(0), Some("foton"));
...
fn item_arguments_propagate_translatable_snbt_errors() {
    init_vanilla_registry();
    for argument in [
        FotonArgumentType::item_stack(),
        FotonArgumentType::item_predicate(),
    ] {
