...
fn coordinate_dispatcher(argument_type: FotonArgumentType) -> TestDispatcher {
...
fn block_position_retains_world_coordinates_until_execution() {
    let dispatcher = coordinate_dispatcher(FotonArgumentType::block_pos());
...
fn vec3_centers_absolute_integer_x_and_z_components() {
    let centered = coordinate_dispatcher(FotonArgumentType::vec3(true));
    let exact = coordinate_dispatcher(FotonArgumentType::vec3(false));
...
fn rotation_argument_retains_yaw_then_pitch_expressions() {
    let dispatcher = coordinate_dispatcher(FotonArgumentType::rotation());
