...
fn score_holder_argument_retains_deferred_names_uuids_selectors_and_wildcards() {
    let single = resource_dispatcher(FotonArgumentType::score_holder());
    ...
    let multiple = resource_dispatcher(FotonArgumentType::score_holders());
...
fn objective_and_integer_range_arguments_retain_vanilla_values() {
    let objective = resource_dispatcher(FotonArgumentType::objective());
    ...
    let range = resource_dispatcher(FotonArgumentType::int_range());
...
fn block_predicate_argument_parses_blocks_tags_properties_and_nbt() {
    ...
    let dispatcher = resource_dispatcher(FotonArgumentType::block_predicate());
    ...
    let Some(oak_x) = foton_registry::REGISTRY
        .blocks
...
fn nbt_path_argument_retains_vanilla_path_nodes() {
    let dispatcher = resource_dispatcher(FotonArgumentType::nbt_path());
    let parse = dispatcher.parse(
...
fn swizzle_argument_retains_unique_axes_and_rejects_duplicates() {
    let dispatcher = resource_dispatcher(FotonArgumentType::swizzle());
