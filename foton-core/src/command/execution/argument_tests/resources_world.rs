...
fn biome_or_tag_argument_resolves_registry_entries_and_tags() {
    ...
    let dispatcher = resource_dispatcher(FotonArgumentType::biome_or_tag());
...
fn structure_or_tag_key_argument_defers_registry_resolution_until_execution() {
    init_vanilla_registry();
    let dispatcher = resource_dispatcher(FotonArgumentType::structure_or_tag_key());
...
fn heightmap_argument_accepts_vanilla_live_world_names_and_suggests_them() {
    let dispatcher = resource_dispatcher(FotonArgumentType::heightmap());
...
fn domain_argument_resolves_and_suggests_only_configured_domains() {
    let dispatcher = resource_dispatcher(FotonArgumentType::domain());
...
fn world_argument_retains_relative_and_fully_qualified_names() {
    let dispatcher = resource_dispatcher(FotonArgumentType::world());
...
fn storage_key_argument_parses_and_suggests_source_domain_keys() {
    let dispatcher = resource_dispatcher(FotonArgumentType::storage_key());
    let parse = dispatcher.parse("resource foton:data", TestSource::new());
    let Ok(chain) = dispatcher.context_chain(parse) else {
...
fn game_mode_argument_parses_only_vanilla_names() {
    let dispatcher = resource_dispatcher(FotonArgumentType::game_mode());
...
fn entity_anchor_argument_parses_and_suggests_vanilla_names() {
    let dispatcher = resource_dispatcher(FotonArgumentType::entity_anchor());
...
fn summonable_entity_argument_resolves_only_registered_factories() {
    init_globals_once();
    let dispatcher = resource_dispatcher(FotonArgumentType::summonable_entity());
...
fn enchantment_argument_resolves_and_suggests_registered_entries() {
    init_vanilla_registry();
    let dispatcher = resource_dispatcher(FotonArgumentType::enchantment());
