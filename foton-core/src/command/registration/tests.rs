...
use crate::command::{
    ...
    execution::{
        CommandResultCallback, ExecutionCommandSource, FotonCommandRuntime, argument, literal,
    },
};
...
fn unique_commands_do_not_pollute_the_root_with_namespaced_variants() {
    let dispatcher = build([
        ...
        command(Identifier::from_foton("fly"), "second"),
    ]);
...
fn explicit_permission_expressions_replace_id_derivation() {
    let permission = PermissionKey::parse("foton.command.inspect");
...
fn derived_subcommand_permissions_follow_literals_through_arguments() {
    let registration = CommandRegistration::new(Identifier::from_foton("perms"), |_| {
        literal("perms").then(
            literal("user").then(
    ...
    let verbose_only = permission_source([permission_entry(
        "foton.command.perms.user.info.verbose",
        PermissionState::Allow,
    )]);
    assert!(
        dispatcher
...
fn explicit_root_permissions_cannot_mix_with_derived_subcommands() {
    let Ok(permission) = PermissionKey::parse("foton.command.tick") else {
