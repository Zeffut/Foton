...
use super::{
    ...
    execution::{CommandPermissionSource, FotonCommandRuntime},
};
...
type CommandFactory<S> =
    dyn FnOnce(NodeId) -> CommandNodeBuilder<S, FotonCommandRuntime> + Send + 'static;
    ...
    S: CommandPermissionSource,
{
    ...
    pub(crate) fn new(
        id: Identifier,
        factory: impl FnOnce(NodeId) -> CommandNodeBuilder<S, FotonCommandRuntime> + Send + 'static,
    ) -> Self {
        Self {
            id,
            aliases: Vec::new(),
...
where
    S: CommandPermissionSource,
{
    pub(crate) dispatcher: CommandDispatcher<S, FotonCommandRuntime>,
    ...
    S: CommandPermissionSource,
{
    ...
    pub(crate) fn build(
        self,
    ) -> Result<CommandDispatcher<S, FotonCommandRuntime>, CommandRegistrationError> {
...
fn apply_registration_requirements<S>(
    mut root: CommandNodeBuilder<S, FotonCommandRuntime>,
    id: &Identifier,
...
) -> Result<CommandNodeBuilder<S, FotonCommandRuntime>, CommandRegistrationError>
where
    S: CommandPermissionSource,
{
    ...
    S: CommandPermissionSource,
{
    id: Identifier,
    aliases: Vec<Box<str>>,
    root: CommandNodeBuilder<S, FotonCommandRuntime>,
}
...
fn register_renamed_root<S>(
    dispatcher: &mut CommandDispatcher<S, FotonCommandRuntime>,
    root: &CommandNodeBuilder<S, FotonCommandRuntime>,
