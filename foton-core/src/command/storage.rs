//! Persistent command storage isolated by Foton domain.
...
/// Vanilla command storage for one Foton domain.
...
/// Loaded command storages keyed by Foton domain.
...
mod tests {
    ...
    use foton_utils::saved_data::SavedDataManager;
    ...
    fn missing_and_empty_values_match_vanilla_storage_semantics() {
        ...
        let key = Identifier::from_foton("data");
    ...
    async fn persistent_storage_round_trips_binary_nbt() {
        let unique = SystemTime::now()
            ...
            .as_nanos();
        let path = temp_dir().join(format!("foton-command-storage-{unique}"));
        let manager = SavedDataManager::new(Some(&path));
        let storage = CommandStorage::new();
        let key = Identifier::from_foton("nested");
    ...
    fn mutation_after_snapshot_remains_dirty() {
        let storage = CommandStorage::new();
        let first = Identifier::from_foton("first");
        let second = Identifier::from_foton("second");
        let mut value = NbtCompound::new();
    ...
    fn invalid_persisted_nbt_is_rejected() {
        let persistent = PersistentCommandStorage {
            entries: [("foton:data".to_owned(), vec![10])].into_iter().collect(),
        };
