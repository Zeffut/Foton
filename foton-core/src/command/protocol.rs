...
use super::execution::FotonArgumentType;
...
impl CommandArgumentProtocol for FotonArgumentType {
    fn protocol_argument(&self) -> (ProtocolArgumentType, Option<SuggestionType>) {
        FotonArgumentType::protocol_argument(self)
    }
}
...
mod tests {
    ...
    use crate::command::execution::{
        CommandArgumentSource, CommandResultCallback, ExecutionCommandSource, FotonArgumentType,
        FotonCommandRuntime, argument as foton_argument, literal as foton_literal,
    };
    use foton_protocol::packets::game::{
    ...
    #[derive(Clone, Copy)]
    struct FotonTestSource;

    impl CommandArgumentSource for FotonTestSource {}

    impl ExecutionCommandSource for FotonTestSource {
    ...
    fn permission_expression_nodes_project_as_terminal_greedy_strings() {
        let mut dispatcher = CommandDispatcher::<FotonTestSource, FotonCommandRuntime>::new();
        let command = foton_literal::<FotonTestSource>("perms")
            .then(foton_argument::<FotonTestSource>(
                "permission",
                FotonArgumentType::permission_rule(),
            ))
            .then(foton_argument::<FotonTestSource>(
                "metadata",
                FotonArgumentType::permission_metadata(),
            ))
            .then(foton_argument::<FotonTestSource>(
                "group",
                FotonArgumentType::permission_group(true),
            ));
        assert!(dispatcher.register(command).is_ok());

        let packet = command_tree_packet(&dispatcher, &FotonTestSource);
        let Ok(packet) = packet else {
    ...
    #[test]
    fn foton_time_argument_projects_its_minimum() {
        let (argument, suggestions) = FotonArgumentType::time(1).protocol_argument();
        let ProtocolArgumentType::Time { min } = argument else {
            panic!("Foton time parser should project as the protocol time argument");
        };
    ...
    #[test]
    fn foton_position_arguments_project_vanilla_parsers() {
        let (block_pos, block_pos_suggestions) = FotonArgumentType::block_pos().protocol_argument();
        assert!(matches!(block_pos, ProtocolArgumentType::BlockPos));
        assert!(block_pos_suggestions.is_none());

        let (vec3, vec3_suggestions) = FotonArgumentType::vec3(true).protocol_argument();
        assert!(matches!(vec3, ProtocolArgumentType::Vec3));
        assert!(vec3_suggestions.is_none());

        let (rotation, rotation_suggestions) = FotonArgumentType::rotation().protocol_argument();
        assert!(matches!(rotation, ProtocolArgumentType::Rotation));
        assert!(rotation_suggestions.is_none());

        let (anchor, anchor_suggestions) = FotonArgumentType::entity_anchor().protocol_argument();
        assert!(matches!(anchor, ProtocolArgumentType::EntityAnchor));
        assert!(anchor_suggestions.is_none());

        let (swizzle, swizzle_suggestions) = FotonArgumentType::swizzle().protocol_argument();
        assert!(matches!(swizzle, ProtocolArgumentType::Swizzle));
        assert!(swizzle_suggestions.is_none());

        let (heightmap, heightmap_suggestions) = FotonArgumentType::heightmap().protocol_argument();
    ...
    #[test]
    fn foton_entity_arguments_use_vanilla_client_suggestions() {
        for (argument, expected_flags) in [
            (FotonArgumentType::entities(), 0),
            (FotonArgumentType::entity(), 1),
            (FotonArgumentType::players(), 2),
            (FotonArgumentType::player(), 3),
        ] {
    ...
    #[test]
    fn foton_scoreboard_arguments_project_vanilla_parsers() {
        for (argument, expected_flags) in [
            (FotonArgumentType::score_holder(), 0),
            (FotonArgumentType::score_holders(), 1),
        ] {
            let (argument, suggestions) = argument.protocol_argument();
            assert!(matches!(
                argument,
        ...
        let (objective, objective_suggestions) = FotonArgumentType::objective().protocol_argument();
        ...
        let (range, range_suggestions) = FotonArgumentType::int_range().protocol_argument();
        assert!(matches!(range, ProtocolArgumentType::IntRange));
        assert!(range_suggestions.is_none());
    }

    #[test]
    fn foton_biome_argument_projects_vanilla_resource_or_tag_parser() {
        let (argument, suggestions) = FotonArgumentType::biome_or_tag().protocol_argument();
    ...
    #[test]
    fn foton_structure_argument_projects_vanilla_resource_or_tag_key_parser() {
        let (argument, suggestions) = FotonArgumentType::structure_or_tag_key().protocol_argument();
    ...
    #[test]
    fn foton_block_predicate_argument_projects_vanillas_parser() {
        let (argument, suggestions) = FotonArgumentType::block_predicate().protocol_argument();
    ...
    #[test]
    fn foton_nbt_path_argument_projects_vanillas_parser() {
        let (argument, suggestions) = FotonArgumentType::nbt_path().protocol_argument();
    ...
    #[test]
    fn foton_component_argument_projects_vanillas_parser() {
        let (argument, suggestions) = FotonArgumentType::component().protocol_argument();
    ...
    #[test]
    fn foton_storage_key_argument_projects_vanillas_identifier_parser() {
        let (argument, suggestions) = FotonArgumentType::storage_key().protocol_argument();
    ...
    #[test]
    fn foton_world_argument_projects_vanillas_dimension_parser() {
        let (argument, suggestions) = FotonArgumentType::world().protocol_argument();
    ...
    #[test]
    fn foton_game_mode_argument_projects_vanillas_parser() {
        let (argument, suggestions) = FotonArgumentType::game_mode().protocol_argument();
    ...
    #[test]
    fn foton_enchantment_argument_projects_vanillas_registry_parser() {
        let (argument, suggestions) = FotonArgumentType::enchantment().protocol_argument();
    ...
    #[test]
    fn foton_item_stack_argument_asks_the_server_for_vanilla_parser_suggestions() {
        let (argument, suggestions) = FotonArgumentType::item_stack().protocol_argument();
    ...
    #[test]
    fn foton_item_predicate_argument_asks_the_server_for_vanilla_parser_suggestions() {
        let (argument, suggestions) = FotonArgumentType::item_predicate().protocol_argument();
    ...
    #[test]
    fn foton_domain_argument_uses_server_resource_suggestions() {
        let (domain, suggestions) = FotonArgumentType::domain().protocol_argument();
    ...
    fn summonable_entity_argument_projects_vanillas_resource_and_suggestions() {
        let (entity, suggestions) = FotonArgumentType::summonable_entity().protocol_argument();
        assert!(matches!(
            entity,
            ProtocolArgumentType::Resource {
    ...
    #[test]
    fn foton_clock_arguments_project_vanilla_resource_parsers() {
        let (clock, clock_suggestions) = FotonArgumentType::world_clock().protocol_argument();
        assert!(matches!(
            clock,
            ProtocolArgumentType::Resource {
        ...
        let (timeline, timeline_suggestions) =
            FotonArgumentType::timeline(Some("clock")).protocol_argument();
        assert!(matches!(
            timeline,
            ProtocolArgumentType::Resource {
        ...
        let (marker, marker_suggestions) = FotonArgumentType::time_marker(None).protocol_argument();
