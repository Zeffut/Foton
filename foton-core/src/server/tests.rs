...
use foton_protocol::packets::game::CRemovePlayerInfo;
use foton_protocol::utils::ConnectionProtocol;
use foton_registry::entity_type::EntityTypeRef;
use foton_registry::item_stack::ItemStack;
use foton_registry::packets::play::{C_ADD_ENTITY, C_PLAYER_INFO_UPDATE, C_SYSTEM_CHAT};
use foton_registry::{
...
fn single_thread_pool() -> Arc<rayon::ThreadPool> {
    Arc::new(
        rayon::ThreadPoolBuilder::new()
            .num_threads(1)
            .build()
            .expect("test thread pool should initialize"),
    )
}
...
async fn test_server_with_worlds(
    ...
    Ok(Arc::new(Server {
        ...
        generation_pool: single_thread_pool(),
        chunk_encoding_pool: single_thread_pool(),
...
fn portal_job_validity_rechecks_vanilla_portal_eligibility() {
    ...
    runtime.block_on(async {
        ...
        player.set_sleeping_pos(foton_utils::BlockPos::new(0, 64, 0));
        assert!(!portal_entity_still_valid(&entity, &world, pending_token));
...
fn execute_as_entity_transform_uses_receiver_with_initiator_permissions() {
    ...
    runtime.block_on(async {
        ...
        let invsee = PermissionExpr::key(permission_key("foton.command.invsee"));
        let modify_key = permission_key("foton.command.invsee.modify");
...
fn command_gameplay_availability_tracks_exact_domain_residence() {
    ...
    runtime.block_on(async {
        ...
        assert_eq!(
            ...
            Ok(2),
            "administrative profile selectors span Foton domains"
        );
        assert_eq!(
            spatial_selector
                .find_online_profile_players(&source)
