...
//!
//! The full deserialization struct lives in the `foton` crate. Foton-core only
...
impl StorageSelection {
    ...
    pub fn default_world_disk() -> Self {
        Self {
            kind: Identifier::from_foton("disk"),
            config: None,
        }
    }
    ...
    pub fn default_player_file() -> Self {
        Self {
            kind: Identifier::from_foton("file"),
            config: None,
        }
    }
...
/// Validates a config path as a relative path under a Foton-owned root.
...
fn validate_player_storage_selection(selection: &StorageSelection) -> Result<(), String> {
    ...
    if selection.config.is_some() {
        return Err("foton:file player storage does not accept config yet".to_owned());
    }
    Ok(())
}
