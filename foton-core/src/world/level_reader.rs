...
use foton_registry::fluid::FluidRef;
...
/// Foton combines those surfaces so the same block behavior can run against a live
...
pub trait LevelAccessor: ScheduledTickAccess {
    ...
    /// as it writes the block. Foton's worldgen region only records a pending marker, so a
