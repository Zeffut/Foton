...
//! [`foton_registry::advancement::predicate`] holds the plain data; this is the
...
//!
//! Anything Foton cannot answer fails closed. A predicate that silently passes
...
use foton_registry::data_components::components::BannerPatternLayers;
...
use foton_registry::loot_table::LootWorldView as _;
use foton_registry::{REGISTRY, RegistryExt as _, TaggedRegistryExt as _};
...
impl PredicateContext<'_> {
    ...
    fn entity_relations_match(&self, predicate: &EntityPredicate, entity: &dyn Entity) -> bool {
        ...
        // view vector plus `ServerPlayer.hasLineOfSight`. Foton's equivalent
    ...
    /// `getOnPos()` is `getOnPos(1.0E-5F)`, which is exactly the offset Foton's
    ...
    pub fn matches_location(&self, predicate: &LocationPredicate, x: f64, y: f64, z: f64) -> bool {
        ...
        // Foton has no structure-at-position lookup at all, so a `structures`
    ...
    /// [`foton_registry::advancement::predicate::DamagePredicate`] because no
    ...
    ///
    /// Foton stores ids rather than references, so an id that no longer resolves
...
fn flags_match(flags: EntityFlagsPredicate, entity: &dyn Entity) -> bool {
    ...
    // Vanilla reads `Entity.isSprinting()` off the shared entity flags; Foton
...
/// carries; Foton reaches the three variants vanilla advancement data actually
...
fn entity_component_matches(component: &EntityComponentMatch, entity: &dyn Entity) -> bool {
    match component.component {
        ...
        // `EntityPartialComponentsPredicate` is open-ended and Foton has no
...
/// Foton can express. The one vanilla use (the ominous banner's name) is unique
