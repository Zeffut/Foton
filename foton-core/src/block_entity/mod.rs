...
//!
//! ```ignore
//! use foton_core::block_entity::{init_block_entities, BLOCK_ENTITIES};
//!
...
use foton_utils::{BlockPos, BlockStateId, ErasedType, locks::SyncMutex};
use simdnbt::borrow::{
...
/// allowing Foton's world ticker to store heterogeneous entries.
#[derive(Clone, Copy)]
...
impl BlockEntityBase {
    ...
    /// near the block; Foton has no such recheck yet, so a close for a menu the
...
/// implementations must claim a unique [`foton_utils::DowncastTypeKey`] through
/// [`foton_utils::DowncastType`].
pub trait BlockEntity: ErasedType + Send + Sync + 'static {
    ...
    ///
    /// Storage and section locks are not held during this callback. This is Foton's staged
    ...
    ///
    /// Vanilla parity: `BlockEntity.saveWithoutMetadata`. Foton keeps the type
    ...
    /// own. Vanilla's `getDefaultName` lives on the block entity; in Foton the
