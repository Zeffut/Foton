...
use foton_registry::loot_table::{LootContext, LootFillContainer, LootTableRef};
...
/// Lets [`foton_registry::loot_table::LootTable::fill`] place rolled loot into
/// any Foton container.
///
/// `LootTable` lives in `foton-registry`, which cannot see [`Container`], so
