...
use foton_registry::blocks::properties::IntProperty;
use foton_registry::blocks::properties::{BlockStateProperties, Direction, EnumProperty};
use foton_registry::data_components::components::{BeehiveOccupant, Bees, EntityData};
use foton_registry::data_components::{DataComponentMap, vanilla_components};
...
/// it.
///
/// Vanilla parity: `BeehiveBlockEntity.IGNORED_BEE_TAGS`. Foton's bee saves a
...
// SAFETY: This key is owned by Foton and uniquely identifies `BeehiveBlockEntity`.
unsafe impl DowncastType for BeehiveBlockEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:block_entity/beehive");
}
...
/// `Occupant.createEntity`. Foton builds the entity from the registry first and
...
mod tests {
    ...
    fn a_bee_goes_into_a_hive_and_comes_back_out_with_the_honey_it_carried() {
        // The whole point of the bee: Foton has had beehives since the start and
        ...
        let released = world.get_entities_in_aabb_matching(
            &foton_utils::WorldAabb::new(4.0, 60.0, 4.0, 13.0, 69.0, 13.0),
