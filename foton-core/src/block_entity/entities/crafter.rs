...
///
/// Vanilla hands the menu the block entity's own `ContainerData`; Foton keeps
...
// SAFETY: This key is owned by Foton and uniquely identifies `CrafterBlockEntity`.
unsafe impl DowncastType for CrafterBlockEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:block_entity/crafter");
}
...
unsafe impl DowncastType for CrafterContainer {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:container/crafter");
}
...
impl CrafterBlockEntity {
    ...
    /// the smallest box holding every item. Foton's shaped matcher slides the
...
impl Container for CrafterContainer {
    ...
    fn can_place_item(&self, slot: usize, _stack: &ItemStack) -> bool {
        ...
        // empty stack reports a stack limit of 64; Foton's reports 1, so that
