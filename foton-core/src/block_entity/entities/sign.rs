...
use foton_registry::{DyeColor, vanilla_block_entity_types};
use foton_utils::{BlockPos, BlockStateId, DowncastType, DowncastTypeKey, locks::SyncMutex};
use simdnbt::borrow::{
...
impl SignText {
    ...
    /// caller learns nothing happened; Foton's `SignText` is edited in place, so
    ...
    /// player's chat filtering setting. Foton's `SignText` keeps no filtered
...
// SAFETY: This key identifies Foton's shared sign implementation for both sign
// registry entries, rather than either registry entry itself.
unsafe impl DowncastType for SignBlockEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:block_entity/sign");
}
...
impl SignBlockEntity {
    ...
    /// report no change. Foton's `SignText` is edited in place, so `edit` says
    ...
    /// followed by `Level.sendBlockUpdated`. Foton sends block entity data as
