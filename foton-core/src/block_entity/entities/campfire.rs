...
use foton_registry::{REGISTRY, vanilla_block_entity_types, vanilla_game_events};
...
// SAFETY: This key is owned by Foton and uniquely identifies `CampfireBlockEntity`.
unsafe impl DowncastType for CampfireBlockEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:block_entity/campfire");
}
...
impl CampfireBlockEntity {
    ...
    /// `result.isItemEnabled` guard has no counterpart: Foton has no feature
    ...
    /// the held stack here through `ItemStack.consumeAndReturn`; Foton's caller
