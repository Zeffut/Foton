...
use foton_utils::{BlockPos, BlockStateId, DowncastType, DowncastTypeKey, UuidExt as _, WorldAabb};
...
struct ConduitState {
    ...
    frame_blocks: usize,
    /// Vanilla `destroyTarget`, an `EntityReference<LivingEntity>`. Foton keeps
...
// SAFETY: This key is owned by Foton and uniquely identifies `ConduitBlockEntity`.
unsafe impl DowncastType for ConduitBlockEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:block_entity/conduit");
}
...
/// one with `closerThan`. Foton walks the world's player list instead, the way
