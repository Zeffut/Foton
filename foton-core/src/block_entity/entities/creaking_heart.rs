...
//! UUID up in the level. Foton keeps the UUID alone and does that lookup, which
...
use foton_registry::blocks::properties::{BlockStateProperties, CreakingHeartState, Direction};
use foton_registry::particle_type::{ParticleData, TrailParticleOption};
use foton_registry::vanilla_block_tags::BlockTag;
use foton_registry::vanilla_game_rules::SPAWN_MONSTERS;
use foton_registry::{
...
// SAFETY: This key is owned by Foton and uniquely identifies `CreakingHeartBlockEntity`.
unsafe impl DowncastType for CreakingHeartBlockEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:block_entity/creaking_heart");
}
