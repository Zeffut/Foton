...
/// Foton-specific fallback for block entity types whose runtime behavior is not implemented yet.
///
/// Vanilla has concrete classes for every block entity type. Foton uses this only to preserve
...
unsafe impl DowncastType for RawBlockEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:block_entity/raw");
}
...
impl BlockEntity for RawBlockEntity {
    ...
    /// block entity type Foton has not implemented yet.
