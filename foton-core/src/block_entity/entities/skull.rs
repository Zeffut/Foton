...
use foton_registry::data_components::vanilla_components::ResolvableProfile;
...
// SAFETY: This key is owned by Foton and uniquely identifies `SkullBlockEntity`.
unsafe impl DowncastType for SkullBlockEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:block_entity/skull");
}
...
impl SkullBlockEntity {
    ...
    /// `NoteBlock.getCustomSoundId`. Foton's note block cannot use it yet: the
