...
use foton_registry::blocks::properties::{BlockStateProperties, StructureMode};
use foton_registry::structure::LiquidSettingsData;
use foton_registry::structure_processor::StructureProcessorKind;
...
use foton_worldgen::structure::{StructureBlockIgnore, StructureMirror as PlacementMirror};
use glam::IVec3;
...
unsafe impl DowncastType for StructureBlockEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:block_entity/structure_block");
}
...
impl StructureBlockEntity {
    ...
    /// level's `StructureTemplateManager`. Foton's manager side is the bundled vanilla
    ...
    fn place_loaded_structure(&self, world: &Arc<World>, template: &StructureTemplate) {
        ...
        // "pick one", which vanilla does from the wall clock and Foton does from the
        ...
        let settings = StructurePlaceSettings {
            ...
            // Vanilla leaves the box unset here, which means "no limit"; Foton's
