...
//! the beam from the glass it can already see, so Foton keeps only the half of
...
// SAFETY: This key is owned by Foton and uniquely identifies `BeaconBlockEntity`.
unsafe impl DowncastType for BeaconBlockEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:block_entity/beacon");
}
...
impl BlockEntity for BeaconBlockEntity {
    ...
    /// pyramid every eighty. Foton does both on the same eighty-tick beat: the
...
#[cfg(test)]
mod tests {
    use foton_registry::{init_vanilla_registry, vanilla_mob_effects};
