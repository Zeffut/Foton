...
// SAFETY: This key is owned by Foton and uniquely identifies `SculkCatalystBlockEntity`.
unsafe impl DowncastType for SculkCatalystBlockEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:block_entity/sculk_catalyst");
}
...
impl BlockEntity for SculkCatalystBlockEntity {
    ...
    fn tick(&self, world: &Arc<World>) {
        ...
        // Foton's spreader is written against `WorldgenRandom` because world generation
...
mod tests {
    ...
    /// The catalyst is the first block entity in Foton to publish a game-event listener, and
