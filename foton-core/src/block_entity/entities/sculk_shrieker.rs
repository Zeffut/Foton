...
use foton_registry::vanilla_game_rules::SPAWN_WARDENS;
use foton_registry::{
...
// SAFETY: This key is owned by Foton and uniquely identifies `SculkShriekerBlockEntity`.
unsafe impl DowncastType for SculkShriekerBlockEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:block_entity/sculk_shrieker");
}
...
/// also resolves a thrown item's owner; Foton's `ItemEntity` stores that owner as a UUID
