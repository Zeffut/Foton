...
fn proto_entities_roundtrip_and_promote_to_full_chunk() {
    ...
    assert!(crystal.add_tag("foton:test".to_owned()));
...
fn unimplemented_block_entities_preserve_nbt_through_proto_save_load() {
    ...
    let proto = Chunk::new(single_empty_section(), pos, 0, 16, Weak::new());
    // A structure block on purpose: it is one of the types Foton still has no concrete
...
const TEST_SPEED_MODIFIER: Identifier = Identifier::new_static("foton", "test_speed");
