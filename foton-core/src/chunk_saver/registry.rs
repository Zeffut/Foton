...
impl WorldStorageRegistry {
    /// Creates a registry containing Foton's built-in world storage backends.
    pub fn new_with_builtins() -> Result<Self, String> {
        let mut registry = Self {
            factories: FxHashMap::default(),
        };
        registry.register(
            Identifier::from_foton("disk"),
            WorldStorageFactory {
...
fn validate_disk_config(config: &toml::Value) -> Result<(), String> {
    let parsed: DiskStorageConfig = config
        .clone()
        .try_into()
        .map_err(|e| format!("invalid foton:disk config: {e}"))?;
