...
use foton_utils::{ChunkPos, Downcast as _, DowncastType, DowncastTypeKey, locks::SyncMutex};
...
unsafe impl DowncastType for ActivationRecordingBlockEntity {
    const TYPE_KEY: DowncastTypeKey =
        DowncastTypeKey::new("foton:test/block_entity/activation_recording");
}
...
fn insertion_rejects_an_entity_owned_by_another_chunk() {
    ...
    // Jigsaw has a vanilla block entity type but no Foton factory yet, so the
...
fn stale_no_entity_promotion_cannot_consume_a_replacement_marker() {
    ...
    // Jigsaw has no Foton block entity factory yet, so the replacement keeps the
