...
use crate::behavior::{
    BLOCK_BEHAVIORS, BlockEntityCreation, FLUID_BEHAVIORS, update_from_neighbour_shapes,
};
...
impl FullChunkRef<'_> {
        ...
        for (section_index, packed_offsets) in postprocessing.into_vec().into_iter().enumerate() {
            ...
            for packed in packed_offsets {
                ...
                if state.get_block().config.liquid {
                    ...
                    let new_state = update_from_neighbour_shapes(world, state, pos);
                    if new_state != state {
    ...
    fn commit_pending_creation(
        ...
        match creation {
            ...
            // Keep Foton's marker until this block gains a factory. Vanilla consumes it because
    ...
    ///
    /// Intentional `Unimplemented` markers remain packed until Foton gains their block factory.
