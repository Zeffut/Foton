...
use foton_registry::{REGISTRY, RegistryEntry, RegistryExt, sound_events, vanilla_blocks};
use foton_registry::{vanilla_damage_types, vanilla_items};
use foton_utils::random::legacy_random::LegacyRandom;
use foton_utils::types::{GameType, InteractionHand, UpdateFlags};
use foton_utils::value_providers::IntProvider;
use foton_utils::{BlockLocalAabb, BlockPos, BlockStateId, Identifier, WorldAabb, axis::Axis};
use glam::DVec3;
...
pub trait BlockBehavior: Send + Sync {
    ...
    /// Foton passes the placement source instead of a borrowed stack so the
    ...
    /// plant-to-pot map in the constructor; Foton has no constructor to hook,
    ...
    /// `cropBlock.isMaxAge(state)`. Foton has no class hierarchy to ask, so a
    ...
    /// Foton returns the velocity to apply so movement resolution keeps entity
    ...
    /// The result distinguishes a missing Foton implementation from a Vanilla factory that
    ...
    /// chest. Foton keeps the halves as separate lockable containers, so a
    ...
    /// across transformations within their block family. Foton checks that
    ...
    /// next state in four different ways. Foton asks the block rather than
