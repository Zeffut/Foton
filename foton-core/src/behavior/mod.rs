...
//! in foton-registry to maintain a clean separation between constant data and
//! functional/dynamic behavior.
//!
//! # Architecture
//!
//! After the main registry (`foton-registry`) is frozen, behavior registries
//! are created:
...
//!
//! ```ignore
//! use foton_core::behavior::{init_behaviors, BLOCK_BEHAVIORS, ITEM_BEHAVIORS};
//!
...
///
/// This is separate from `BlockStateExt` (in foton-registry) because these methods
/// require access to the behavior registry which lives in foton-core.
pub trait BlockStateBehaviorExt {
    ...
    /// Returns whether this block state is a crop, and if so whether it is ripe.
    fn crop_is_max_age(&self) -> Option<bool>;
...
impl BlockStateBehaviorExt for BlockStateId {
    ...
    fn crop_is_max_age(&self) -> Option<bool> {
        let block = self.get_block();
        BLOCK_BEHAVIORS.get_behavior(block).crop_is_max_age(*self)
    }
...
mod parity_ledger_tests {
    ...
    /// The committed record of what vanilla Foton does not implement.
