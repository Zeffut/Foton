...
use foton_registry::instrument::InstrumentValue;
...
/// Vanilla parity: `Instrument.range() / 16.0F`. Foton's sound range is derived
...
fn play(context: &UseItemContext, instrument: &InstrumentValue) {
    ...
    // packet carries a registry id, so Foton cannot send one yet -- the same
...
mod tests {
    ...
    use foton_registry::{init_vanilla_registry, vanilla_instruments, vanilla_items};
    ...
    fn the_note_carries_as_far_as_the_instruments_range() {
        ...
        // Vanilla asks for range/16 as the volume, and Foton's sound range is
