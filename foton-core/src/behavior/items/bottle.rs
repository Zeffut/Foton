...
//!
//! Two of vanilla's three ways to fill a bottle are unreachable in Foton today:
//!
//! - The dragon-breath branch of `BottleItem.use` collects area effect clouds
//!   whose `getOwner()` is an `EnderDragon`. Foton has an area effect cloud
...
//!   lives in the `CauldronInteraction.WATER` map, which Foton has not ported
...
use foton_registry::fluid::FluidStateExt;
