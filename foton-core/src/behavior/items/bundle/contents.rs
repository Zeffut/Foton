...
//!
//! Vanilla keeps `BundleContents.Mutable` inside the component itself. Foton
...
//! `foton-core`, while the component lives in `foton-registry`. The weight rule
//! therefore stays with the component and the editing lives here.

use foton_registry::ItemStackTemplate;
use foton_registry::data_components::{BundleContents, CheckedFraction};
...
impl MutableBundleContents {
    ...
    ///
    /// Foton deviation: the resulting stack is checked against the persistent
