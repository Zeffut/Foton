...
//!
//! Not ported here: `getCloneItemStack`, because Foton's signature has neither
//! world nor position and so cannot reach the block entity the sherds live on;
//! the `Stats.ITEM_USED` award of `useItemOn`, because Foton has no statistics
...
use foton_registry::data_components::vanilla_components::POT_DECORATIONS;
...
impl BlockBehavior for DecoratedPotBlock {
    ...
    fn player_will_destroy(
        ...
        // Vanilla's flag 260 is Foton's `UPDATE_NONE`: the pot is about to be
    ...
    ///
    /// Foton has no loot tables, so both alternatives are resolved here. The
    ...
    /// which Foton has no counterpart for.
    fn use_item_on(
        &self,
        _state: BlockStateId,
        world: &Arc<World>,
...
mod tests {
    ...
    use foton_registry::data_components::vanilla_components::ENCHANTMENTS;
    use foton_registry::{
