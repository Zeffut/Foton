...
use foton_registry::data_components::vanilla_components::{CUSTOM_NAME, NOTE_BLOCK_SOUND, PROFILE};
...
use foton_utils::{BlockPos, BlockStateId, Downcast as _, Identifier};
...
/// property of the block rather than of the block entity. Foton's loot engine
...
/// and the model the client draws, both of which Foton takes from generated
...
///
/// The class exists in vanilla only to widen the collision shape, and Foton
...
#[cfg(test)]
mod tests {
    use foton_registry::data_components::vanilla_components::{PlayerSkinPatch, ResolvableProfile};
    ...
    fn a_head_remembers_whose_it_is_from_placement_to_breaking() {
        ...
        let owner = profile_of("Fotonhead");
    ...
    fn a_skeleton_skull_gives_back_its_name_but_never_a_profile() {
        ...
        item.set(PROFILE, profile_of("Fotonhead"));
