...
//! Foton cannot do (see the block entity's own note). Corner mode's `unloadStructure`
...
use foton_registry::blocks::properties::StructureMode;
...
fn trigger(world: &Arc<World>, block_entity: &StructureBlockEntity) {
    match block_entity.mode() {
        StructureMode::Save => {
            log::debug!(
                "structure block at {:?} was triggered to save, which Foton cannot do yet",
