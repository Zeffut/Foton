...
/// block-state data. Structure rotation and mirroring use Foton's common facing
...
impl BlockBehavior for ChiseledBookShelfBlock {
                ...
                contents
                    ...
                    .map(|item| {
                        item.as_ref().map_or_else(
                            ItemStack::empty,
                            foton_registry::ItemStackTemplate::create,
                        )
                    })
                    .collect::<Vec<_>>()
            })
    ...
    fn use_item_on(
        ...
        // Foton does not yet have the item-used stat foundation.
...
mod tests {
    ...
    use foton_utils::types::{GameType, UpdateFlags};
