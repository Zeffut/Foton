...
impl BlockBehavior for NoteBlock {
    ...
    fn use_without_item(
        ...
        // The tune-noteblock stat awaits Foton's shared statistics foundation.
    ...
    fn attack(&self, state: BlockStateId, world: &Arc<World>, pos: BlockPos, player: &Player) {
        ...
        // The play-noteblock stat awaits Foton's shared statistics foundation.
    }
    ...
    fn trigger_event(
        ...
        let Some(sound) = Self::sound_event(instrument) else {
            ...
            // Foton's sound packet carries only registered sound events, so
