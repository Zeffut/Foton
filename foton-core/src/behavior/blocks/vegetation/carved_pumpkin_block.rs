...
use foton_registry::{blocks::BlockRef, level_events, vanilla_blocks, vanilla_entities};
...
///
/// Vanilla creates the entity and then calls `snapTo`; Foton's entity factory
...
///
/// Foton has no `CopperChestBlock` behavior yet, so the two parts of vanilla's
