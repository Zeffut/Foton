...
//!
//! Vanilla holds `Set<Raider>` -- strong references to the mobs. Foton holds
...
use foton_math::trig;
use foton_protocol::packets::game::{BossBarColor, BossBarOverlay, CSound, SoundSource};
...
///
/// Vanilla parity: `Raid.RaidStatus`, renamed because Foton already calls the
...
pub struct Raid {
    ...
    ///
    /// Vanilla looks this up with `Raids.getId` by identity. Foton stores it,
...
impl Raid {
    ...
    pub fn absorb_raid_omen(&self, player: &Player) -> bool {
        ...
        // `RAID_OMEN` advancement trigger when this is the first wave. Foton
        // has neither system.
        true
    }
    ...
    pub fn update_boss_bar(&self, world: &World) {
        ...
        // reloaded before its first wave -- puts a NaN on the bar. Foton draws
    ...
    /// back; Foton keys on entity id, which a reload changes, so the stale id
    ...
    pub fn join_raid(
        ...
        // Vanilla sets the position here; Foton builds the entity at the spawn
    ...
    fn play_raid_horn(&self, world: &Arc<World>, sound_origin: BlockPos) {
        ...
        world.players.iter_players(|_, player| {
            ...
            // Foton plays it where they are instead rather than sending a NaN
