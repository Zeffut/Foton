...
//! stored per dimension under `data/raids.dat`. Foton writes the same thing to
//! `data/raids.toml` through [`SavedDataManager`](foton_utils::saved_data::SavedDataManager),
...
use foton_registry::vanilla_game_rules::RAIDS;
...
impl Raids {
    ...
    /// Foton hands it out before the raid is built.
    ...
    pub fn tick(&self, world: &Arc<World>) {
        ...
        // Vanilla marks the saved data dirty every two hundred ticks; Foton
