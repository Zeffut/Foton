...
//!
//! Verifies that Foton's chunk generation matches vanilla Minecraft at each stage
...
use foton_registry::structure::TerrainAdjustment;
use foton_registry::{dimension_type::DimensionTypeRef, vanilla_dimension_types};
use foton_utils::types::{Difficulty, GameType};
use foton_utils::{ChunkPos, Identifier};
use foton_worldgen::noise::Beardifier;
use foton_worldgen::structure::StructureStart;
use glam::IVec3;
...
/// Prefer `FOTON_HASH_DEBUG_CLUSTER` for light failures because fixtures are
/// captured after the extractor lights whole sampled clusters.
const DEBUG_CHUNKS: &[(i32, i32)] = &[];
const DEBUG_CLUSTER_ENV: &str = "FOTON_HASH_DEBUG_CLUSTER";
const DEBUG_CHUNK_ENV: &str = "FOTON_HASH_DEBUG_CHUNK";
const DEBUG_DIMENSION_ENV: &str = "FOTON_HASH_DEBUG_DIMENSION";
const DEBUG_STAGE_ENV: &str = "FOTON_HASH_DEBUG_STAGE";
const DEBUG_STOP_AFTER_FIRST_MISMATCH_ENV: &str = "FOTON_HASH_STOP_AFTER_FIRST_MISMATCH";
...
fn compute_block_hash(sections: &Sections) -> String {
    ...
    for section_holder in &sections.sections {
        ...
        // cave_air / void_air. Foton's palette-level `has_only_air()` doesn't
...
fn describe_state(state_id: i32) -> String {
    ...
    use foton_utils::types::BlockStateId;
        ...
        match ref_section {
            Some(Some(ref_ids)) => {
                if section.states.has_only_air() {
                    // Foton says all air, vanilla has data
            ...
            Some(None) | None => {
                // Vanilla says all air (or section missing). Check if Foton also has air.
                if !section.states.has_only_air() {
                    for y_local in 0..16usize {
                        for z in 0..16usize {
                            for x in 0..16usize {
    ...
    for (section_index, reference_section) in reference.iter().enumerate() {
        ...
        let Some(actual_section) = actual.get(section_index) else {
            push_light_diff(
                diffs,
                format!(
                    "{layer_name} section y={section_y}: vanilla={} foton=missing-section",
        ...
        if actual_state != reference_section.state {
            push_light_diff(
                diffs,
                format!(
                    "{layer_name} section y={section_y}: vanilla={} foton={}",
                    describe_light_state(reference_section.state),
        ...
        for index in 0..4096 {
            ...
            let foton = light_value(actual_bytes, index);
            if vanilla == foton {
                continue;
            }
            ...
            push_light_diff(
                diffs,
                format!(
                    "{layer_name} section y={section_y} ({local_x:2},{local_y:2},{local_z:2}): vanilla={vanilla} foton={foton}"
                ),
            );
        }
    }
...

    {
        ...
        for z in 0..16 {
            for x in 0..16 {
                ...
                let foton = sky_sources.get_lowest_source_y(x, z);
                if vanilla != foton {
                    push_light_diff(
                        &mut diffs,
                        format!("sky source ({x:2},{z:2}): vanilla={vanilla} foton={foton}"),
                    );
                }
            }
        }
