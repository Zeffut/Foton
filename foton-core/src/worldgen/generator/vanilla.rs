...
use foton_math::lerp2;
use foton_registry::biome::BiomeRef;
use foton_registry::blocks::block_state_ext::BlockStateExt;
use foton_registry::carver::ConfiguredCarverKind;
use foton_registry::{REGISTRY, RegistryEntry, RegistryExt, vanilla_biomes};
use foton_utils::random::{
    Random, RandomSource, RandomSplitter, legacy_random::LegacyRandom, xoroshiro::Xoroshiro,
};
use foton_utils::{BlockPos, BlockStateId, ChunkPos, DowncastType, DowncastTypeKey, Identifier};
use foton_worldgen::density::{ColumnCache, DimensionNoises, NoiseSettings};
use foton_worldgen::density_functions::{
    end::EndNoises, nether::NetherNoises, overworld::OverworldNoises,
};
use foton_worldgen::noise_parameters::get_noise_parameters;
use foton_worldgen::surface::{
...
use foton_worldgen::biomes::obfuscate_biome_seed;
use foton_worldgen::noise::Beardifier;
use foton_worldgen::noise::NoiseChunk;
use foton_worldgen::noise::OreVeinifier;
use foton_worldgen::noise::{Aquifer, AquiferResult, LazyAquifer, preliminary_surface_level};
use foton_worldgen::structure::{GenerationContext, TerrainHeightSampler};
...
///
/// Custom dimension noise implementations own their state type and downcast key. Foton
...
/// Foton-owned post-Noise state for the built-in dimensions.
#[doc(hidden)]
pub struct FotonPostNoiseState<N: DimensionNoises> {
    aquifer: Aquifer<N>,
}

// SAFETY: Each key uniquely identifies this exact Foton-owned specialization.
unsafe impl DowncastType for FotonPostNoiseState<OverworldNoises> {
    const TYPE_KEY: DowncastTypeKey =
        DowncastTypeKey::new("foton:worldgen_state/post_noise_overworld");
}

// SAFETY: Each key uniquely identifies this exact Foton-owned specialization.
unsafe impl DowncastType for FotonPostNoiseState<NetherNoises> {
    const TYPE_KEY: DowncastTypeKey =
        DowncastTypeKey::new("foton:worldgen_state/post_noise_nether");
}

// SAFETY: Each key uniquely identifies this exact Foton-owned specialization.
unsafe impl DowncastType for FotonPostNoiseState<EndNoises> {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:worldgen_state/post_noise_end");
}

macro_rules! impl_post_noise_state_type {
    ($noises:ty) => {
        impl VanillaPostNoiseStateType for $noises {
            type State = FotonPostNoiseState<Self>;

            fn wrap_post_noise_aquifer(aquifer: Aquifer<Self>) -> Self::State {
                FotonPostNoiseState { aquifer }
            }
...
pub struct VanillaGenerator<N: DimensionNoises> {
    ...
    ///
    /// Vanilla still samples each source biome in `apply_carvers`; Foton skips
...
impl<N: VanillaPostNoiseStateType> ChunkGenerator for VanillaGenerator<N> {
    ...
    fn initial_spawn_search_origin(&self) -> foton_utils::BlockPos {
...
mod tests {
    ...
    fn blocks(chunk: &Chunk) -> Vec<foton_utils::BlockStateId> {
    ...
    fn has_overworld_post_noise_state(chunk: &Chunk) -> bool {
        chunk
            .with_transient_generation_state_mut::<
                super::FotonPostNoiseState<super::OverworldNoises>,
                _,
            >(|_| ())
            .is_some()
    }
