...
use foton_registry::dimension_type::DimensionTypeRef;
use foton_registry::vanilla_biomes;
use foton_registry::vanilla_dimension_types::{OVERWORLD, THE_END, THE_NETHER};
...
use foton_worldgen::structure::placement::load_vanilla_structure_sets;
...
impl WorldGeneratorRegistry {
    /// Creates a registry containing Foton's built-in generator factories.
...
fn validate_empty_world_config(config: &toml::Value) -> Result<WorldGeneratorConfigData, String> {
    let parsed: DimensionTypeOnlyConfig = config
        .clone()
        .try_into()
        .map_err(|e| format!("invalid foton:empty config: {e}"))?;
...
fn create_empty(
    ...
    let WorldGeneratorConfigData::EmptyWorld(parsed) = config else {
        return Err("validated config does not match foton:empty".to_owned());
    };
    let dimension_type = dimension_type_by_key(&parsed.dimension_type)?;
    Ok(GeneratorOutput {
        dimension_type,
