...
use foton_utils::random::RandomSource;
use foton_utils::{
...
/// chunks already collected by Foton's `StaticCache2D` and validated against `ChunkStep`.
...
/// block states directly. Foton keeps chunk views cached instead of section references because
...
impl<'a> WorldGenRegion<'a> {
    ...
    /// entity. If Foton does not have concrete behavior for the type yet, the raw fallback keeps
...
impl<'region, 'world, 'profile> WorldGenBulkSectionAccess<'region, 'world, 'profile> {
    ...
    /// block entity callbacks, and other `WorldGenRegion.setBlock` side effects. Foton also
...
impl LevelAccessor for WorldGenRegion<'_> {
    ...
    fn attach_block_entity(
        ...
        let _ = self.set_block_entity_data(pos, block_entity_type, state, NbtCompound::new());
    }
}
