...
use foton_registry::blocks::properties::{BlockStateProperties, RailShape};
use foton_registry::blocks::shapes::SupportType;
use foton_registry::vanilla_biome_tags::BiomeTag;
use foton_registry::{Registry, RegistryExt, vanilla_blocks, vanilla_entities};
use foton_utils::axis::Axis;
use foton_utils::random::Random;
use foton_utils::random::worldgen_random::WorldgenRandom;
use foton_utils::{BlockPos, BlockStateId, BoundingBox, Direction, Identifier, types::UpdateFlags};
use glam::DVec3;
...
impl<L: WorldGenLevel> MineshaftPlacer<'_, L> {
    ...
    fn is_mineshaft_blocking_biome(&self, pos: BlockPos) -> bool {
        let Some(biome_id) = fuzzed_biome_at_block(self.biome_zoom_seed, pos, |quart| {
            ...
            panic!("mineshaft biome check reached a column the region has no chunk for");
        };
