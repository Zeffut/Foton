...
use foton_core::bootstrap::init_globals_once;
use foton_core::chunk::Chunk;
use foton_core::chunk::chunk_generation_task::StaticCache2D;
use foton_core::chunk::chunk_holder::ChunkHolder;
use foton_core::chunk::chunk_map::ChunkMap;
use foton_core::chunk::chunk_pyramid::{ChunkDependencies, ChunkStep, GENERATION_PYRAMID};
use foton_core::chunk::chunk_status_tasks::ChunkStatusTasks;
use foton_core::chunk::chunk_ticket_manager::ChunkTicketLevel;
use foton_core::chunk::section::{ChunkSection, Sections};
...
use foton_core::worldgen::generator::generation_benchmark_support;
use foton_core::worldgen::{
...
use foton_registry::dimension_type::DimensionType;
...
use foton_worldgen::biomes::{BiomeSourceKind, ChunkBiomeSampler};
...
static FEATURE_BATCH_PROFILE_LOG_LIMIT: LazyLock<u64> = LazyLock::new(|| {
    env::var("FOTON_FEATURE_BATCH_PROFILE_LOG_LIMIT")
        .ok()
...
static FULL_PIPELINE_PROFILE_LOG_LIMIT: LazyLock<u64> = LazyLock::new(|| {
    env::var("FOTON_FULL_PIPELINE_PROFILE_LOG_LIMIT")
        .ok()
        .and_then(|value| value.parse().ok())
        .unwrap_or(16)
});
...
static LIGHT_PROFILE_LOG_LIMIT: LazyLock<u64> = LazyLock::new(|| {
    env::var("FOTON_LIGHT_PROFILE_LOG_LIMIT")
        .ok()
        .and_then(|value| value.parse().ok())
        .unwrap_or(16)
});
...
fn bench_overworld_features_concurrent_overlap(c: &mut Criterion) {
    ...
    c.bench_function("overworld_generate_features_concurrent_overlap", |b| {
        b.iter_batched(
            ...
            |fixture| {
                if env::var_os("FOTON_FEATURE_BATCH_PROFILE").is_some() {
...
fn run_concurrent_full_pipeline_batch(fixture: ConcurrentFullPipelineFixture) {
    let mut stage_times = env::var_os("FOTON_FULL_PIPELINE_PROFILE")
        .is_some()
        .then(Vec::new);
    let batch_started_at = Instant::now();
...
fn run_concurrent_light_batch(fixture: ConcurrentLightFixture) {
    let profile = env::var_os("FOTON_LIGHT_PROFILE").is_some();
    let batch_started_at = Instant::now();
    let stage_started_at = Instant::now();
...
fn bench_noise_kernel(c: &mut Criterion) {
    ...
    use foton_worldgen::noise::ImprovedNoise;
