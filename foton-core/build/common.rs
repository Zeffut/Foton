...
///
/// Handles both `#[item_behavior]` and `#[foton_macros::item_behavior]`.
...
pub(crate) enum JsonArgKind {
    ...
    /// `module_path` is the full import path (e.g. `"foton_registry::blocks::properties"`).
    Enum {
        type_name: String,
...
fn int_provider_to_tokens(
    ...
    if provider.is_number() {
        ...
        return quote! { foton_utils::value_providers::IntProvider::Constant(#value) };
    }
    ...
    quote! {
        foton_utils::value_providers::IntProvider::Uniform {
            min_inclusive: #min,
            max_inclusive: #max,
        }
    }
