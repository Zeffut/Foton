...
impl TranspileContext {
    pub(super) fn gen_noises_struct(&self) -> TokenStream {
        ...
        let blended_field = self.blended_noise_config.as_ref().map(|_| {
            quote! { pub blended_noise: foton_worldgen::noise::BlendedNoise, }
        });

        let end_islands_field = if self.uses_end_islands {
            Some(quote! { pub end_islands: foton_worldgen::noise::EndIslands, })
        } else {
            None
        };
    ...
    pub(super) fn gen_noises_impl(&self) -> TokenStream {
        ...
        let field_inits: Vec<TokenStream> = self
            ...
            .map(|id| {
                ...
                if legacy && id == "minecraft:temperature" {
                    quote! {
                        #field: {
                            let mut rng = foton_worldgen::random::RandomSource::Legacy(
                                foton_worldgen::random::legacy_random::LegacyRandom::from_seed(seed)
                            );
                    ...
                    quote! {
                        #field: {
                            let mut rng = foton_worldgen::random::RandomSource::Legacy(
                                foton_worldgen::random::legacy_random::LegacyRandom::from_seed(seed.wrapping_add(1))
                            );
                            NormalNoise::create_legacy_nether_biome(&mut rng, -7, &[1.0, 1.0])
                        }
                    }
            ...
            if legacy {
                ...
                quote! {
                    blended_noise: {
                        ...
                        foton_worldgen::noise::BlendedNoise::new(
                            &mut rng,
                ...
                quote! {
                    blended_noise: {
                        use foton_worldgen::random::PositionalRandom;
                        use foton_worldgen::random::name_hash::NameHash;
        ...
        let end_islands_init = if self.uses_end_islands {
            Some(quote! {
                end_islands: foton_worldgen::noise::EndIslands::new(seed),
            })
        } else {
            None
        };
