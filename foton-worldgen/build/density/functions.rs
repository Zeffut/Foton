...
const DATAPACK_BASE: &str = "../foton-utils/build_assets/builtin_datapacks/minecraft/worldgen";
...
fn generate_noise_settings(dimension: &str, prefix: &str) -> TokenStream {
    ...
    let (
        ...
        (
            ...
            quote! {
                {
                    static BLOCK_STATES: std::sync::OnceLock<Box<[foton_utils::BlockStateId]>> =
                        std::sync::OnceLock::new();
                    BLOCK_STATES.get_or_init(|| {
                        Box::from([
                            #(foton_registry::vanilla_blocks::#block_state_idents.default_state()),*
                        ])
                    })
                }
            },
        ...
        let empty_func = quote! {
            ...
            fn apply_surface_rule_impl(
                _ctx: &mut foton_worldgen::surface::SurfaceRuleContext<'_>,
            ) -> Option<foton_utils::BlockStateId> {
                None
            }
        };
        (
    ...
    quote! {
        ...
        impl #settings_struct {
            ...
            #[inline]
            pub fn default_block_id() -> foton_utils::BlockStateId {
                foton_registry::REGISTRY.blocks.get_default_state_id(&foton_registry::vanilla_blocks::#default_block_ident)
            }

            /// Get the default fluid state ID for this dimension.
            #[inline]
            pub fn default_fluid_id() -> foton_utils::BlockStateId {
                foton_registry::REGISTRY.blocks.get_default_state_id(&foton_registry::vanilla_blocks::#default_fluid_ident)
            }
        }

        impl foton_worldgen::density::NoiseSettings for #settings_struct {
            ...
            #[inline]
            fn default_block_id() -> foton_utils::BlockStateId {
                #settings_struct::default_block_id()
            }

            #[inline]
            fn default_fluid_id() -> foton_utils::BlockStateId {
        ...
        impl foton_worldgen::density::ColumnCache for #cache_struct {
        ...
        impl foton_worldgen::density::DimensionNoises for #noises_struct {
            type ColumnCache = #cache_struct;
            type Settings = #settings_struct;

            fn create(
                seed: u64,
                splitter: &foton_utils::random::RandomSplitter,
                params: &rustc_hash::FxHashMap<String, foton_worldgen::density::NoiseParameters>,
            ) -> Self {
            ...
            fn surface_rule_block_states() -> &'static [foton_utils::BlockStateId] {
                #surface_block_states_tokens
            }
            ...
            fn try_apply_surface_rule(
                ctx: &mut foton_worldgen::surface::SurfaceRuleContext<'_>,
            ) -> Option<foton_utils::BlockStateId> {
