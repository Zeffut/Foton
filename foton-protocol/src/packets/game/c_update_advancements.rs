...
//! map; Foton keeps ordered vectors, so the same packet always encodes to the
//! same bytes and can be tested against them.

use std::io::{Result, Write};

use foton_macros::ClientPacket;
use foton_registry::advancement::{AdvancementProgress, AdvancementRef, DisplayInfo};
use foton_registry::packets::play::C_UPDATE_ADVANCEMENTS;
use foton_utils::Identifier;
use foton_utils::codec::VarInt;
use foton_utils::serial::{PrefixedWrite as _, WriteTo};
...
#[cfg(test)]
mod tests {
    use foton_registry::advancement::AdvancementProgress;
    use foton_registry::init_vanilla_registry;
    use foton_registry::vanilla_advancements::STORY_ROOT;
