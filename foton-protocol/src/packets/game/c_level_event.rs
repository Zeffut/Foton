...
use foton_registry::packets::play::C_LEVEL_EVENT;
...
///
/// See `foton_registry::level_events` for all available event type constants.
#[derive(WriteTo, ClientPacket, Clone, Debug)]
#[packet_id(Play = C_LEVEL_EVENT)]
pub struct CLevelEvent {
    /// The event type ID. Use constants from `foton_registry::level_events`.
    pub event_type: i32,
...
impl CLevelEvent {
    ...
    pub const fn destroy_block(pos: BlockPos, block_state_id: u32) -> Self {
        Self::new(
            foton_registry::level_events::PARTICLES_DESTROY_BLOCK,
            pos,
