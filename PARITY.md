...
**All three columns are now down to their floor.** The item column is closed but
for vanilla's plain `Item`, which needs nothing. The entity column is closed but
for `Player`, which is not implemented through the behavior mechanism at all.
The block column holds ten entries, four of which are vanilla base classes that
need no behavior either -- `Block`, `AirBlock`, `TransparentBlock`,
`HalfTransparentBlock`. That leaves six real ones: `StainedGlassBlock`,
`TintedGlassBlock`, `CryingObsidianBlock`, `StructureVoidBlock`, and vanilla's
two gametest blocks `TestBlock` and `TestInstanceBlock`.

Which is exactly the point the rest of this document makes. A class count near
zero says the behaviors are *registered*. It says nothing about whether they are
*correct*, and the sections below are the record of how often those two came
apart.
...
by a test. `python3 dev/coverage.py` reads the source tree independently and
must reach the same answer; when the two disagree the ledger is right, because
it is generated by the same codegen that does the registering. That happened:
the script only matched a bare `#[item_behavior]` and missed the two book items
written as `#[foton_macros::item_behavior]`, so it under-reported the item
column by two for as long as those files existed. A gap that closes has to be crossed off deliberately; a gap that
