...
/// to leave on disk. The emitted file opens with a comment saying exactly
/// that, so an operator who opens `config.yml` directly is not surprised the
/// next time an edit they made there does not survive a restart.
///
...
pub fn render_config(options: &GeyserOptions) -> String {
    ...
    format!(
        "\
# Generated by Foton on every start -- edits made here are lost on the next
# one. Configure Bedrock support through [server.bedrock] in Foton's own
# config instead.
bedrock:
  address: 0.0.0.0
  port: {bedrock_port}
...
mod tests {
    ...
    #[test]
    fn the_generated_config_warns_it_will_be_overwritten() {
        // Foton regenerates this file whole on every start (see
        // `render_config`'s own doc comment); an operator who edits it by
        // hand and never sees that stated anywhere loses the edit with no
        // clue why. The warning must be a YAML comment -- not a key -- so it
        // cannot itself break parsing.
        let yaml = render_config(&options());
        assert!(yaml.starts_with("# Generated by Foton"));
        assert!(yaml.contains("[server.bedrock]"));
        for line in yaml.lines().take_while(|line| line.starts_with('#')) {
            assert!(
                line.starts_with("# "),
