...
//! `steel-core` because the trial-spawner config registry is generated data and
//! has to name them.
//!
//! The light check `SpawnData.CustomSpawnRules.isValidPosition` needs a level,
//! so it stays in `steel-core`; what is here is the range pair it reads.
...
/// table Steel does not have is dropped rather than defaulted, so an unknown
