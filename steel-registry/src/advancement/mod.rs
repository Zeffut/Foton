...
//! from the built-in datapack by `steel-registry/build/advancements.rs`, so
//! everything here is plain data plus the codecs the protocol needs. The tree,
//! the per-player bookkeeping and the triggers live in `steel-core`.
...
impl AdvancementIcon {
    ...
    /// item: a tooltip Steel cannot model is not worth refusing to draw the
    ...
    pub fn template(&self) -> &ItemStackTemplate {
        self.template.get_or_init(|| {
            ...
            decode_icon(item, self.components_snbt).unwrap_or_else(|| {
                log::error!(
                    "advancement icon {} has a component patch Steel could not decode: {}",
                    self.item_id,
                    self.components_snbt
                );
...
/// `TreeNodePosition`; Steel's advancement set is fixed at build time, so the
...
/// gives it its id. Steel keeps the id on the value, the way every other
...
pub struct Advancement {
    ...
    /// Whether vanilla reports it to its telemetry service. Steel never does;
