...
fn generate_read_code(
    ...
    match strategy.name_str().as_str() {
        "VarInt" => quote! {
            foton_utils::codec::VarInt::read(data)?.0 as #field_type
        },
        "VarLong" => quote! {
            foton_utils::codec::VarLong::read(data)?.0 as #field_type
        },
        "Prefixed" => {
            let prefix = strategy
                .prefix_type_tokens()
            ...
            if let Some(inner) = &strategy.inner {
                ...
                quote! {
                    {
                        use foton_utils::serial::PrefixedRead;
    ...
    match s.fields {
        Fields::Named(fields) => {
            ...
            let expanded = quote! {
                ...
                impl foton_utils::serial::ReadFrom for #name {
...
fn read_from_enum(e: syn::DataEnum, name: Ident, attrs: Vec<syn::Attribute>) -> TokenStream {
    ...
    let read_discriminant = match &strategy.as_ref().map(Strategy::name_str) {
        ...
        None => {
            quote! { foton_utils::codec::VarInt::read(data)?.into() }
        }
        // Explicit VarInt
        Some(s) if s == "VarInt" => {
            quote! { foton_utils::codec::VarInt::read(data)?.into() }
        ...
        // VarLong
        Some(s) if s == "VarLong" => {
            quote! { foton_utils::codec::VarLong::read(data)?.into() }
        }
        ...
        Some(s) if ALLOWED_TYPES.contains(&s.as_str()) => {
            ...
            quote! { <#enum_type as foton_utils::serial::ReadFrom>::read(data)? }
        }
        Some(s) => panic!(
