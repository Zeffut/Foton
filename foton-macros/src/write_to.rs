...
fn generate_write_code(
    ...
    match strategy.name_str().as_str() {
        "VarInt" => quote! {
            foton_utils::codec::VarInt(#value as i32).write(writer)?;
        },
        "VarLong" => quote! {
            foton_utils::codec::VarLong(#value as i64).write(writer)?;
        },
        "Byte" => quote! {
        ...
        "Json" => {
            let prefix = strategy
                .prefix_type_tokens()
                .unwrap_or_else(|| quote! { foton_utils::codec::VarInt });
            quote! {
                {
                    use foton_utils::serial::PrefixedWrite;
        ...
        "RegistryHolder" => quote! {
            foton_utils::codec::VarInt((#value) as i32 + 1).write(writer)?;
        },
        "Prefixed" => {
            let prefix = strategy
                .prefix_type_tokens()
    ...
    match s.fields {
        Fields::Named(fields) => {
            ...
            let expanded = quote! {
                ...
                impl #impl_generics foton_utils::serial::WriteTo for #name #ty_generics {
...
fn write_to_enum(name: Ident, attrs: Vec<syn::Attribute>) -> TokenStream {
    ...
    let writer = match strategy_name.as_str() {
        ...
        "VarInt" => {
            quote! {
                foton_utils::codec::VarInt(*self as i32).write(writer)?;
            }
        }
    ...
    TokenStream::from(quote! {
        #[automatically_derived]
        impl foton_utils::serial::WriteTo for #name {
            fn write(&self, writer: &mut impl std::io::Write) -> std::io::Result<()> {
                #writer
