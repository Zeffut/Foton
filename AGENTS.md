...
**VERIFY BEFORE ASKING** — Inspect relevant Foton code, generated/extracted data, vanilla source, and tests before asking. If still blocked, say what you checked.
...
- `.clone()` to appease borrow checker, `.unwrap()`/`.expect()` in production, `unsafe` other than the two exceptions named below (keyed `DowncastType`, and `chunk_saver::nesting`)
- `Any`/`TypeId` downcasting in plugin-extensible foundations. It is sound within one linked Rust build, but its identity is not deterministic across builds; use Foton's keyed downcasting foundation instead.
...
**DOWNCASTING** — Exact concrete downcasting uses `foton_utils::{Downcast, DowncastType, DowncastTypeKey}`.
...
- Keys identify concrete Rust implementations, not Minecraft registry entries or translation keys. Foton-owned types use the `foton:` namespace; plugins own their namespace.
- Foton may downcast Foton-owned types and a plugin may downcast its own types. A key does not make a Rust type ABI-stable or permit one owner to reinterpret another owner's private type.
...
**NESTING DEPTH** — `chunk_saver::nesting::Nested` is the second approved
`unsafe` site, and the only one outside downcasting.
- Two persisted types contain themselves, `wincode` has no depth limit, and its
  `SchemaRead`/`SchemaWrite` are `unsafe trait`s -- so a depth ceiling cannot be
  written in safe Rust at all. Without one, a corrupt region file overflows the
  stack, and a stack overflow is a SIGSEGV: no quarantine runs, and the server
  dies again on every restart.
- The exception is narrow on purpose. Both impls do nothing but count a level
  and forward to the inner schema; they leave `TYPE_META` at the default
  `Dynamic`, which the trait documents as always safe, and they touch `dst`
  only through the inner schema's own safe `get`. Widening either impl past
  delegation ends the approval.
- Any further `unsafe` still follows the workaround protocol above.
...
**Particle routing**
- In Vanilla, `Level.addParticle` and `playLocalSound` calls from shared ticks are client-local, so Foton generally has no server-side work to perform for them. An explanatory comment can still be useful when an omission would otherwise look accidental.
...
Foton = Minecraft server in Rust.

**Crates:** `foton` (thin wrapper) -> `foton-login` (initial connection) → `foton-core` (game logic) → `foton-worldgen` (worldgen) → `foton-protocol` (packets) → `foton-macros` (derives) → `foton-registry` (generated data) → `foton-utils`/`foton-math` (common) → `foton-crypto` (encryption)
...
| Area | Path |
|------|------|
| Entry / networking | `foton/src/` |
| Game logic | `foton-core/src/` |
| Player | `foton-core/src/player/` (`networking.rs` = packet handlers) |
| World | `foton-core/src/world/` |
| Worldgen orchestration | `foton-core/src/worldgen/` |
| Worldgen algorithms/data | `foton-worldgen/src/` |
| Chunks | `foton-core/src/chunk/` |
| Block/item behaviors | `foton-core/src/behavior/` |
| Block entities | `foton-core/src/block_entity/` |
| Entities | `foton-core/src/entity/` |
| Inventory / menus | `foton-core/src/inventory/` |
| Packets | `foton-protocol/src/packets/game/` |
| Codegen build scripts | `foton-core/build/`, `foton-registry/build/`, `foton-worldgen/build/`, `foton-utils/build/` |
...
- `foton_utils::locks::SyncMutex<T>` / `foton_utils::locks::SyncRwLock<T>` — aliased parking_lot for easier comprehension
