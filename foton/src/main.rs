...
use foton_core::player::player_data_storage::GlobalPlayerData;
use foton_core::player::player_inventory::MenuRemovalStatus;
use foton_core::server::Server;
use foton_utils::text::DisplayResolutor;
use foton_utils::threading::worker_threads_for_available;
    ...
    S: Subscriber + for<'span> LookupSpan<'span> + Send + Sync,
{
    ...
    let tracer_provider = SdkTracerProvider::builder()
        .with_resource(
            Resource::builder()
                .with_attributes([
                    KeyValue::new("service.name", "foton"),
                    KeyValue::new(
                        "service.build",
    ...
    let tracer = tracer_provider.tracer("foton");
...
fn foton_main() {
    ...
    let main_worker_threads = configured_worker_threads(foton_config.server.threads.main_runtime);
    let chunk_worker_threads = configured_worker_threads(foton_config.server.threads.chunk_runtime);
    ...
    main_runtime.block_on(main_async(chunk_runtime.clone(), foton_config));
...
async fn main_async(chunk_runtime: Arc<Runtime>, foton_config: config::FotonConfig) {
    let cancel_token = CancellationToken::new();

    let logger = match init_tracing(cancel_token.clone(), foton_config.log.clone()).await {
        Ok(logger) => logger,
        Err(error) => {
            eprintln!("{error}");
            return;
    ...
    let run_result = AssertUnwindSafe(run_server(chunk_runtime, cancel_token, foton_config))
        .catch_unwind()
        .await;
...
async fn run_server(
    ...
    let mut foton = FotonServer::new(chunk_runtime.clone(), cancel_token.clone(), foton_config)
        .await
...
async fn shutdown_worlds(server: &Arc<Server>) {
    ...
    match command_data.maps {
        Ok(saved) => log::info!("Saved {saved} domain map stores"),
        Err(error) => log::error!("Failed to save domain map data: {error}"),
    }
    match command_data.boss_bars {
        Ok(saved) => log::info!("Saved {saved} domain boss-bar sets"),
        Err(error) => log::error!("Failed to save domain boss bars: {error}"),
    }
