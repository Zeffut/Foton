    ...
    fn from_codec_tag(tag: NbtTag) -> Option<Self> {
        match tag {
            ...
            NbtTag::String(value) => {
                ...
                let compound = foton_utils::nbt::parse_snbt_compound(&value).ok()?;
...
impl PartialEq for CustomData {
    fn eq(&self, other: &Self) -> bool {
        foton_utils::nbt::nbt_compounds_equal(&self.tag, &other.tag)
    }
}
...
mod tests {
    ...
    fn persistent_codec_accepts_compounds_and_flattened_snbt() {
        ...
        let flattened =
            CustomData::from_nbt_value(&NbtTag::String("{value:7,nested:{name:'foton'}}".into()))
