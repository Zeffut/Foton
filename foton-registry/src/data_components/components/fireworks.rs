...
fn int_list_from_nbt(tag: &NbtTag) -> Option<Vec<i32>> {
    tag.list()?
        .as_nbt_tags()
        .iter()
        .map(foton_utils::nbt::NbtNumeric::codec_i32)
        .collect()
}
...
mod tests {
    ...
    fn unknown_stream_shape_ids_fall_back_to_small_ball() {
        let mut network = Vec::new();
        foton_utils::codec::VarInt(99)
            .write(&mut network)
            .expect("shape should encode");
        foton_utils::codec::VarInt(0)
            .write(&mut network)
    ...
    fn extracted_firework_rocket_has_one_unit_of_flight() {
        ...
        let rocket = REGISTRY
            .items
            .by_key(&foton_utils::Identifier::vanilla_static("firework_rocket"))
