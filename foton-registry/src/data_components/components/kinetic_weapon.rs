...
fn optional_f32<T: foton_utils::nbt::NbtNumeric>(tag: Option<T>, default: f32) -> Option<f32> {
    match tag {
...
mod tests {
    ...
    use foton_utils::hash::HashComponent as _;
    ...
    fn extracted_netherite_spear_keeps_kinetic_thresholds_and_sounds() {
        ...
        let spear = REGISTRY
            .items
            .by_key(&foton_utils::Identifier::vanilla_static("netherite_spear"))
        ...
        assert_eq!(
            kinetic
                ...
                .map(|sound| &sound.key),
            Some(&foton_utils::Identifier::vanilla_static("item.spear.use"))
        );
        assert_eq!(
            kinetic
                .hit_sound()
                .and_then(crate::sound_event::SoundEventHolder::registry_ref)
                .map(|sound| &sound.key),
            Some(&foton_utils::Identifier::vanilla_static("item.spear.hit"))
        );
    }
}
