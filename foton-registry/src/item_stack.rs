...
impl ItemStack {
    ...
    pub fn hurt_and_break_with_random(
        ...
        if self.is_broken() {
            ...
            // An `ItemStack` in `foton-registry` has no way to reach one, so the
    ...
    #[must_use]
    pub fn get_destroy_speed(&self, block_state_id: foton_utils::BlockStateId) -> f32 {
        self.get_tool()
    ...
    #[must_use]
    pub fn is_correct_tool_for_drops(&self, block_state_id: foton_utils::BlockStateId) -> bool {
        self.get_tool()
    ...
    /// origin but no world, and Foton has no structure locator reachable from
    /// `foton-registry` at all, so there is nothing here to search with.
    ///
...
mod persistence_tests {
    ...
    fn untrusted_stack_bytes(
        count: i32,
        component: Option<(&foton_utils::Identifier, Vec<u8>)>,
    ) -> Vec<u8> {
        let mut bytes = Vec::new();
        VarInt(count)
            .write(&mut bytes)
