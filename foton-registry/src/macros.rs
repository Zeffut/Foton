...
macro_rules! impl_registry_ext {
    ($Registry:ty, $Entry:ty, $id_field:ident, $key_field:ident) => {
        impl $crate::RegistryExt for $Registry {
            ...
            fn by_key(&self, key: &foton_utils::Identifier) -> Option<&'static $Entry> {
                self.$key_field
                    .get(key)
                    .and_then(|&id| self.$id_field.get(id).copied())
            }

            fn id_from_key(&self, key: &foton_utils::Identifier) -> Option<usize> {
...
macro_rules! impl_registry_entry {
    ($Entry:ty, $global_field:ident) => {
        ...
        impl $crate::RegistryEntry for $Entry {
            fn key(&self) -> &foton_utils::Identifier {
                &self.key
            }
...
macro_rules! impl_tagged_registry {
    ($Registry:ty, $key_field:ident, $entity_name:literal) => {
        impl $crate::TaggedRegistryExt for $Registry {
            fn register_tag(&mut self, tag: foton_utils::Identifier, keys: &[&'static str]) {
                assert!(
                ...
                let entry_keys = keys
                    .iter()
                    .filter_map(|key| {
                        let ident = foton_utils::registry::registry_vanilla_or_custom_tag(key);
            ...
            fn modify_tag(
                &mut self,
                tag: &foton_utils::Identifier,
                f: impl FnOnce(Vec<foton_utils::Identifier>) -> Vec<foton_utils::Identifier>,
            ) {
                let existing = self
                    .tags
                    .remove(tag)
            ...
            fn is_in_tag(&self, entry: &Self::Entry, tag: &foton_utils::Identifier) -> bool {
                self.tags.contains(tag, $crate::RegistryEntry::key(entry))
            }

            fn get_tag(&self, tag: &foton_utils::Identifier) -> Option<Vec<&'static Self::Entry>> {
            ...
            fn tag_keys(&self) -> impl Iterator<Item = &foton_utils::Identifier> + '_ {
                self.tags.keys()
            }
        }
    };
