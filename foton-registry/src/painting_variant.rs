...
impl WriteTo for PaintingVariantValue {
    fn write(&self, writer: &mut impl Write) -> Result<()> {
        foton_utils::codec::VarInt(self.width).write(writer)?;
        foton_utils::codec::VarInt(self.height).write(writer)?;
...
impl ReadFrom for PaintingVariantValue {
    fn read(data: &mut Cursor<&[u8]>) -> Result<Self> {
        Ok(Self {
            width: foton_utils::codec::VarInt::read(data)?.0,
            height: foton_utils::codec::VarInt::read(data)?.0,
