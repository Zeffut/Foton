...
use foton_utils::{BlockPos, Downcast as _, DowncastType, DowncastTypeKey, ErasedType, Identifier};
...
// SAFETY: This Foton-owned key uniquely identifies the concrete position-source payload.
unsafe impl DowncastType for BlockPositionSource {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:position_source/block");
}
...
unsafe impl DowncastType for EntityPositionSource {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:position_source/entity");
}
