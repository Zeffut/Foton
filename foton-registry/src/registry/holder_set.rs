...
///
/// The trait keeps the holder-set codec independent from Foton's concrete
...
mod tests {
    ...
    fn codecs_reject_unknown_registry_values() {
        ...
        let unknown_tag = NbtTag::String("#foton:missing".into());
        assert_eq!(
            with_borrowed_tag(unknown_tag, RegistryHolderSet::<Item>::from_nbt_tag),
            None
        );
        let unknown_entry = NbtTag::String("foton:missing".into());
        assert_eq!(
    ...
    fn string_codec_rejects_malformed_identifiers() {
        ...
        let missing_tag = Identifier::new_static("foton", "missing");
        assert!(
