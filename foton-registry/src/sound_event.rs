...
// SAFETY: This Foton-owned key uniquely identifies `SoundEventHolder` within
// the linked process.
unsafe impl DowncastType for SoundEventHolder {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("foton:registry/sound_event_holder");
}
...
impl SoundEventHolder {
    ...
    pub(crate) fn from_owned_nbt(tag: &NbtTag) -> Option<Self> {
        ...
        let fixed_range = compound
            .get("range")
            .and_then(foton_utils::nbt::NbtNumeric::codec_f32);
        Some(Self::Direct {
            sound_id,
            fixed_range,
        })
