    ...
    for (block, info) in block_assets.blocks.iter().zip(block_shape_infos.iter()) {
        ...
        let map_color_overwrites = block
            ...
            .map(|overwrite| {
                ...
                let value = overwrite.value;
                quote! {
                    StateMapColorOverwrite::new(#offset, MapColor::from_id(#value))
                }
            })
            .collect::<Vec<_>>();
        ...
        stream.extend(quote! {
            pub static #block_name: Block = Block::new(
                ...
                StateMapColorData::new(
                    MapColor::from_id(#map_color_default),
    ...
    quote! {
        use crate::{
            blocks::{
                ...
                StateBooleanOverwrite, StateFluidData, StateFluidOverwrite, StateMapColorData,
                StateMapColorOverwrite, offset,
            },
        ...
        use foton_utils::{BlockLocalAabb, Identifier};
