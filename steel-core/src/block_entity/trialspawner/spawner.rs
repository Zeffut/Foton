...
use steel_registry::vanilla_game_rules::{SPAWN_MOBS, SPAWNER_BLOCKS_WORK};
use steel_registry::{
...
/// display entity, the ambient sound roll) is absent: Steel is a server, and
...
impl TrialSpawner {
    ...
    fn spawn_mob(
        ...
        let spawn_aabb = steel_utils::WorldAabb::entity_box(
            spawn_pos.x,
            spawn_pos.y,
            spawn_pos.z,
        ...
        if let Some(mob) = entity.as_mob() {
            ...
            // the mob; Steel has no static predicate, so the instance is asked
...
fn packed_pos(pos: BlockPos) -> i64 {
    steel_utils::PackedBlockPos::from(pos).as_raw()
}
