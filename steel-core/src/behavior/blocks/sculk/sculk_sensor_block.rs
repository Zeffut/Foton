...
impl SculkSensorBlock {
    ...
    /// Vanilla `SculkSensorBlock.activate`.
    ///
    /// Nothing in Steel calls this yet: it is the entry point a vibration listener uses
...
///
/// Not implemented: `rotate` and `mirror`. Steel resolves block rotation from extracted
...
impl CalibratedSculkSensorBlock {
    ...
    ///
    /// As with the plain sensor, nothing calls this until Steel has vibrations.
    pub fn activate(
        &self,
        source_entity: Option<&dyn Entity>,
        world: &Arc<World>,
    ...
    /// for. Steel measures it here so the calibration is already readable when a vibration
    /// system arrives.
    #[must_use]
