...
use steel_registry::{sound_events, vanilla_blocks, vanilla_game_events, vanilla_particle_types};
use steel_utils::color::RgbColor;
...
///
/// Steel gap: the ambient idle sound of `animateTick` is a `playLocalSound` and
/// is the client's own business, and `entityInside` -- the poison a bee walks
/// into -- waits on Steel dispatching that hook.
#[block_behavior]
