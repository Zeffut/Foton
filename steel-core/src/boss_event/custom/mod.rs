...
//! reaches it through the server. Steel's scoreboards and command storage are
...
/// The named boss bars of one Steel domain.
///
...
impl CustomBossEvents {
    ...
    /// which is Steel's stand-in for the `setDirty` callback vanilla's
...
/// Loaded boss bars keyed by Steel domain.
