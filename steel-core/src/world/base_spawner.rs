...
//!
//! Two structural differences, both forced by Steel and both documented where
//! they bite:
//!
//! * Vanilla asks `SpawnPlacements.checkSpawnRules` before it creates anything.
//!   Steel has no path from an entity type to its spawn predicate, so the mob is
...
/// `displayEntity`) are absent: Steel is a server, and vanilla only ever writes
...
impl BaseSpawner {
    ...
    ///
    /// Vanilla keeps `spawnDelay` private; Steel exposes it because a test has
    ...
    /// `Level.hasNearbyAlivePlayer`. Steel has no such method; the nearest
    ...
    fn try_spawn_one(
        ...
        // Vanilla builds the entity from the tag here. Steel has no dispatch on
        ...
        let Some(entity) = load_spawner_entity(
            ...
            // spawner. Steel gets here when the entity type has no factory,
...
///
/// Vanilla counts by exact Java class; Steel counts by entity type, the finest
...
///
/// Vanilla parity: `EntityType.loadEntityRecursive`. Steel resolves the type
