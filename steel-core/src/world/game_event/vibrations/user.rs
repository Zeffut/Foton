...
use steel_registry::position_source::{
...
use steel_registry::{REGISTRY, TaggedRegistryExt as _, vanilla_position_source_types};
...
///
/// Vanilla parity: `PositionSource`, the level-side interface with `getPosition`. Steel's
/// [`steel_registry::position_source::PositionSource`] is only that interface's network
...
pub trait VibrationUser: Send + Sync {
    ...
    /// here, because Steel has no advancement criteria system.
