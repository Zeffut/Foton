...
//! would pick the melee goal every time anyway. Steel has no hostile-mob
...
//! modeled; see the TODOs below for the goals that depend on mob types Steel
...
use steel_registry::{sound_events, vanilla_attributes, vanilla_mob_effects};
...
// SAFETY: This key is owned by Steel and uniquely identifies `WitherSkeletonEntity`.
...
mod tests {
    ...
    /// the entries that need mob types Steel does not implement yet.
    #[test]
