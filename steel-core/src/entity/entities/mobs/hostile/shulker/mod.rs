...
//!
//! **Gaps** in this port, all of them in Steel's foundations rather than the
//! mob:
//!
//! - `Shulker.makeBoundingBox` grows the hitbox out of the attach face as the
//!   lid opens, and `onPeekAmountChange` shoves whatever the lid meets. Steel's
...
//!   team, and Steel has no teams, so it is left out rather than written as a
//!   goal that can never fire.
//! - `Shulker.move` turns a `MoverType.SHULKER_BOX` push into a teleport.
//!   Nothing in Steel pushes with that mover type yet, and `Entity::move_entity`
...
///
/// Vanilla parity: `AABB.contract`, which Steel's uniform `deflate` cannot
...
// SAFETY: This key is owned by Steel and uniquely identifies `ShulkerEntity`.
...
impl Entity for ShulkerEntity {
    ...
    /// to a wall or ceiling grows into the block it is facing, which Steel's
