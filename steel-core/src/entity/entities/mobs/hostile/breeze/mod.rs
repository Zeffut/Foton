...
//! 75. Steel's look control has no per-mob head rotation speed yet -- the same
...
// SAFETY: This key is owned by Steel and uniquely identifies `BreezeEntity`.
...
impl Entity for BreezeEntity {
    ...
    /// *applied*; Steel's `ProjectileDeflection` is a plain enum with no sound
    ...
    /// deflect a second time. And none of it is reachable yet: no Steel mob is
