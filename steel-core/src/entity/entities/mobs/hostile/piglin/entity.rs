...
use steel_registry::data_components::vanilla_components::KINETIC_WEAPON;
...
// SAFETY: This key is owned by Steel and uniquely identifies `PiglinEntity`.
    ...
    pub fn finish_conversion(&self) -> Option<Arc<ZombifiedPiglinEntity>> {
        ...
        convert_to(
            ...
            |zombified| {
                ...
                // `ConversionType.SINGLE.convert`; Steel's conversion leaves
...
impl Mob for PiglinEntity {
    ...
    /// `onItemPickup`, which only fires an advancement trigger Steel has no
