...
//! makes. Steel has no entity class hierarchy, so the whole of the guardian --
...
use steel_utils::{BlockPos, Downcast as _, DowncastType};
...
pub(super) struct GuardianState {
    ...
    /// reaches into directly through the reference it keeps. Steel's goals live
...
/// water. Steel's `PathfinderMob::get_walk_target_value` default only
...
///
/// Vanilla parity: `LevelReader.canSeeSkyFromBelowWater`. Steel has no such
