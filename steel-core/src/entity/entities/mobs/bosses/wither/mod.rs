...
// SAFETY: This key is owned by Steel and uniquely identifies `WitherBoss`.
...
impl WitherBoss {
    ...
    fn rejects_damage(&self, world: &World, source: &DamageSource) -> bool {
        ...
        // Steel has no `AbstractArrow` layer, so its two concrete subclasses
...
impl LivingEntity for WitherBoss {
    ...
    /// Steel routes every effect through `can_be_affected`, so the blanket
