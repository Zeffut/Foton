...
// SAFETY: This key is owned by Steel and uniquely identifies `EnderDragon`.
...
impl LivingEntity for EnderDragon {
    ...
    ///
    /// Steel has no separate killing-blow hook, so the override sits on `die`
    ...
    fn ai_step(&self) -> Option<MoveResult> {
        ...
        // Vanilla parity: `this.hurtTime == 0`. Steel carries only
