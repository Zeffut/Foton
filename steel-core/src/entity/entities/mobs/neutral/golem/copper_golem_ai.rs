...
/// previous and next nodes, and Steel's `Path` exposes those, but the door half
        ...
        vec![
            (
                ...
                Behavior::boxed(TransportItemsBetweenContainers::new(
                    ...
                    // open. Steel's `ContainerOpenersCounter` port is a plain
    ...
    Box::new(
        move |body: &dyn PathfinderMob, target: &TransportItemTarget, ticks: i32| {
            ...
            if ticks == TARGET_INTERACTION_TIME {
                ...
                // openers; Steel's counter has no opener list, so the golem's
