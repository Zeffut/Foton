...
//! of the ghast, and Steel has no dismount-location hook to hang that on -- the
...
//! spawn at one, which Steel's spawner has no per-entity hook for; that is the
...
//! told to hold still: Steel has no path from an entity to its tracker, so the
...
use steel_registry::{REGISTRY, TaggedRegistryExt as _, sound_events, vanilla_attributes};
...
pub struct HappyGhastEntity {
    ...
    /// the control object. Steel recreates its controls each tick, so the state
    /// they carry lives on the mob.
    float_duration: SyncMutex<i32>,
}

// SAFETY: This key is owned by Steel and uniquely identifies `HappyGhastEntity`.
...
impl HappyGhastEntity {
    ...
    /// moment the timeout starts. Steel has no path from an entity to its
...
impl Mob for HappyGhastEntity {
    ...
    /// mount. Steel's interaction dispatch already runs the equippable branch
