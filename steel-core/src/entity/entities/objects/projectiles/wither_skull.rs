...
// SAFETY: This key is owned by Steel and uniquely identifies `WitherSkullEntity`.
unsafe impl DowncastType for WitherSkullEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("steel:entity/wither_skull");
}
...
impl Projectile for WitherSkullEntity {
    ...
    fn on_hit_entity(&self, entity: &SharedEntity, _location: DVec3) {
        ...
        let was_hurt = if let Some(owner) = living_owner {
            ...
            // `EnchantmentHelper.doPostAttackEffects`; Steel has no post-attack
    ...
    /// blocks a plain one bounces off. Steel's `World::explode` has no
...
mod tests {
    ...
    use glam::DVec3;
    use steel_registry::{init_vanilla_registry, vanilla_entities, vanilla_mob_effects};
