...
unsafe impl DowncastType for LingeringPotionEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("steel:entity/lingering_potion");
}
...
impl LingeringPotionEntity {
    ...
    ///
    /// Vanilla also calls `cloud.setOwner(owner)`. Steel's
...
mod tests {
    ...
    use steel_utils::{BlockPos, ChunkPos, Direction, Identifier};
    ...
    /// The entity decides, not the bottle. Steel used to read the carried item
