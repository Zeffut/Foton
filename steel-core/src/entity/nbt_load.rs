...
///   one straight back, because the world indexes a live entity by it. Steel
...
pub(crate) fn load_live_entity(entity: &dyn Entity, nbt: BorrowedNbtCompoundView<'_, '_>) {
    ...
    if let Err(error) = entity.try_set_position(position) {
        // Vanilla's `setPosRaw` cannot fail. Steel's commits through the world
...
/// own. Steel keeps the two halves as separate methods, so the order is spelt
