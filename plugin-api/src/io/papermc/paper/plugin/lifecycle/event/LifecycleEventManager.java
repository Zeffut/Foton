...
public final class LifecycleEventManager {
    ...
    public <T> void registerEventHandler(LifecycleEventType<T> type, LifecycleEventHandler<T> handler) {
        handlers.add(new Registration<>(type, handler));
    }
    public <T> void registerEventHandler(LifecycleEventHandlerConfiguration<T> configuration) {
        handlers.add(new Registration<>(null, configuration.handler()));
    }
    @SuppressWarnings("unchecked")
    public <T> void dispatch(LifecycleEventType<T> type, T event) {
    ...
    private record Registration<T>(LifecycleEventType<T> type, LifecycleEventHandler<T> handler) {}
}
